...
async function runTests() {
  ...
  try {
    ...
    await test('serves waiting page when no screens exist', async () => {
      ...
      assert(
        res.body.includes('Waiting for Claude'),
    ...
    await test('handles multiple concurrent WebSocket clients', async () => {
      ...
      fs.writeFileSync(
        path.join(TEST_DIR, 'multi-client.html'),
        '<h2>Multi</h2>',
      )
      await sleep(500)
    ...
    await test('cleans up closed clients from broadcast list', async () => {
      ...
      fs.writeFileSync(
        path.join(TEST_DIR, 'after-close.html'),
        '<h2>After</h2>',
      )
      await sleep(300)
    ...
    await test('clears .events on new screen', async () => {
      ...
      fs.writeFileSync(
        path.join(TEST_DIR, 'clear-events.html'),
        '<h2>New screen</h2>',
      )
      await sleep(500)
      ...
      assert(
        !fs.existsSync(eventsFile),
        '.events should be cleared on new screen',
      )
    })
