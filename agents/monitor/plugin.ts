    ...
    api.registerService({
      ...
      start: async () => {
        ...
        if (useStandalone) {
          ...
          serverProcess = spawn(
            npx,
            ['next', 'start', '-p', String(config.port), '-H', config.host!],
            {
              cwd: pluginDir,
              env,
              stdio: ['ignore', 'pipe', 'pipe'],
        ...
        serverProcess.on('exit', (code) => {
          if (!stopping) {
            api.logger.warn(`[agent-monitor] Server exited with code ${code}`)
          }
          serverProcess = null
        })

        serverProcess.on('error', (err) => {
          api.logger.error(`[agent-monitor] Failed to start: ${err.message}`)
          serverProcess = null
        })
