...
const AGENT_AVATARS: AgentConfig['avatar'][] = [
  'glasses',
  'hoodie',
  'suit',
  'casual',
...
function normalizeSendPolicy(
  policy: GatewaySessionInfo['sendPolicy'],
): 'allow' | 'deny' | 'unknown' {
...
function sessionToAgentConfig(
  sess: GatewaySessionInfo,
  index: number,
): AgentConfig {
  ...
  return {
    ...
    name: sess.name,
    emoji:
      sess.emoji ?? (isMain ? '⚡' : AGENT_EMOJIS[index % AGENT_EMOJIS.length]),
...
export function useAgents(forceDemoMode = false): UseAgentsReturn {
  ...
  const [agents, setAgents] = useState<AgentConfig[]>(
    forceDemoMode ? DEMO_AGENTS : [],
  )
  const [agentStates, setAgentStates] = useState<
    Record<string, AgentDashboardState>
  >({})
  ...
  const agentMetaRef = useRef<
    Record<string, Pick<AgentConfig, 'name' | 'emoji'>>
  >({})
  ...
  const addGlobalMessage = useCallback((message: ChatMessage) => {
    setGlobalChatMessages((prev) =>
      appendUniqueMessage(prev, message, MAX_GLOBAL_MESSAGES),
    )
  }, [])
  ...
  const addThreadMessages = useCallback(
  ...
  const fetchSessions = useCallback(async (): Promise<boolean> => {
    ...
    try {
      ...
      const newAgents = sortedSessions.map((sess, i) =>
        sessionToAgentConfig(sess, i),
      )
      setAgents(newAgents)
      ...
      setSystemStats({
        ...
        totalTasks: sortedSessions.filter((s) =>
          isActiveBehavior((s.behavior ?? 'idle') as AgentBehavior),
        ).length,
        completedTasks: 0,
        failedTasks: sortedSessions.filter((s) => s.aborted).length,
        totalBroadcasts: 0,
        activeThreads: sortedSessions.filter(
          (s) => (s.childSessionIds?.length ?? 0) > 0,
        ).length,
        uptime: 0,
        connected: true,
      })
    ...
    const connectEventStream = () => {
      ...
      es.addEventListener('state', (evt) => {
        try {
          ...
          const tool =
            data.toolName || data.toolPhase
              ? {
                  toolName: data.toolName ?? null,
                  toolPhase: data.toolPhase ?? null,
                }
          ...
          setAgentStates((prev) => {
            ...
            if (data.agentStatus === 'error') {
              ...
              data.agentStatus === 'assistant' &&
              data.chatStatus === 'final'
            ) {
              const meta = agentMetaRef.current[sessionId]
              messageEvent = {
            ...
            return {
              ...
              [sessionId]: {
                ...
                currentTask: isActiveBehavior(behavior)
                  ...
                  : null,
                totalTasks: isActiveBehavior(behavior)
                  ? Math.max(existing.totalTasks, 1)
                  : existing.totalTasks,
                sessionLog: [
                  ...existing.sessionLog.filter(
                    (line) =>
          ...
          return {
            ...
            [sessionId]: {
              ...
              sessionLog: [
                ...existing.sessionLog.filter(
                  (line) =>
                    !line.startsWith('Status: ') && !line.startsWith('Tool: '),
                ),
                `Status: ${statusSummary}`,
                ...(tool.toolName
                  ? [
    ...
    demoTimerRef.current = setInterval(() => {
      setAgentStates((prev) => {
        ...
        if (agent && next[agent.id]) {
          const newBehavior =
            DEMO_BEHAVIORS[Math.floor(Math.random() * DEMO_BEHAVIORS.length)]
          next[agent.id] = {
            ...next[agent.id],
            ...
            lastActivity: Date.now(),
            totalTokens:
              next[agent.id].totalTokens + Math.floor(Math.random() * 500),
          }
        }
        return next
      })
  ...
  useEffect(() => {
    setSystemStats((prev) => ({
      ...prev,
      mainAgents:
        agents.filter((agent) => !agent.isSubagent).length || prev.mainAgents,
      subAgents: agents.filter((agent) => !!agent.isSubagent).length,
      totalBroadcasts: globalChatMessages.filter(
        (m) => m.scope === 'broadcast' && m.role === 'user',
      ).length,
      activeThreads: agents.filter(
        (agent) => (agent.subagentIds?.length ?? 0) > 0,
      ).length,
    }))
  }, [agents, globalChatMessages])

  const setBehavior = useCallback(
    (agentId: string, behavior: AgentBehavior) => {
      setAgentStates((prev) => {
  ...
  const pollForReply = useCallback(
    async (agentId: string, key: string, scope: ChatScope = 'direct') => {
      ...
      setChatMessages((prev) => ({
        ...
        [agentId]: (prev[agentId] ?? []).map((message) =>
          message.id === thinkingId
            ? {
                ...message,
                content: 'No reply yet (agent may still be processing)',
                isThinking: false,
                timestamp: Date.now(),
              }
            : message,
      ...
      try {
        ...
        for (const msg of rawMessages) {
          ...
          converted.push({
            ...
            content: displayContent,
            timestamp:
              Date.now() - (rawMessages.length - converted.length) * 1000,
            scope: 'history',
            channel: 'agent',
          })
        }
        ...
        setTimeout(
          () => {
            const reply = createChatMessage({
              ...
              content:
                DEMO_CHAT_RESPONSES[
                  Math.floor(Math.random() * DEMO_CHAT_RESPONSES.length)
                ],
              scope: 'direct',
              channel: 'agent',
            })
  ...
  const sendGlobalChat = useCallback(
    async (message: string, sender?: Pick<OwnerConfig, 'name' | 'emoji'>) => {
      ...
      const targetEntries = targets
        ...
        .filter(
          (entry): entry is { target: AgentConfig; key: string } =>
      ...
      try {
        ...
        const deliveredKeys = new Set<string>(
          (data.delivered ?? []).map(
            (item: { sessionKey: string }) => item.sessionKey,
          ),
        )
        await Promise.all(
          targetEntries
            ...
            .map((entry) =>
              pollForReply(entry.target.id, entry.key, 'broadcast'),
            ),
        )
      } catch {
        addGlobalMessage(
