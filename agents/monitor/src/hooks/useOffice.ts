  ...
  const tickFn = useCallback(() => {
    setOfficeState((prev) => {
      ...
      for (const runtime of prev.agents) {
        ...
        if (zone) {
          const inZone =
            updated.pos.col >= zone.minCol &&
            updated.pos.col <= zone.maxCol &&
            updated.pos.row >= zone.minRow &&
            updated.pos.row <= zone.maxRow
        ...
        if (updated.path.length > 0) {
          ...
          if (
            mapping.bubble &&
            newTick % 300 ===
              (agents.indexOf(agents.find((a) => a.id === runtime.id)!) * 50) %
                300
          ) {
            const sp = gridToScreen(updated.pos)
            newBubbles.push({
        ...
        updated.currentState =
          behavior === 'working' || behavior === 'debugging'
            ? 'working'
            : behavior === 'thinking'
              ? 'thinking'
