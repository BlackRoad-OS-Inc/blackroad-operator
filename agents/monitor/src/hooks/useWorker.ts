...
export function useCanvasRenderer(config?: {
  ...
  updateConfig: (
    config: Partial<
      import('../workers/canvas-renderer.worker').IsometricConfig
    >,
  ) => void
  isReady: boolean
} {
  ...
  const pendingResolve = useRef<
    Map<
      string,
      (
        value: import('../workers/canvas-renderer.worker').RenderCalculation,
      ) => void
    >
  >(new Map())
  ...
  const calculateRender = useCallback(
    (
      ...
      import('../workers/canvas-renderer.worker').RenderCalculation
    > => {
