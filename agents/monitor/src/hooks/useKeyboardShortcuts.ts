...
export function useKeyboardShortcuts({
  ...
  const checkVimSequence = useCallback(
    (key: string): KeyboardShortcut | null => {
      ...
      for (const shortcut of allShortcuts()) {
        if (
          shortcut.sequence &&
          shortcut.sequence.join('') === newSequence.join('')
        ) {
          return shortcut
        }
      }
      ...
      const hasPartialMatch = allShortcuts().some(
        (s) =>
          s.sequence &&
          s.sequence.slice(0, newSequence.length).join('') ===
            newSequence.join(''),
      )
  ...
  const updateSettings = useCallback(
    (newSettings: Partial<ShortcutSettings>) => {
