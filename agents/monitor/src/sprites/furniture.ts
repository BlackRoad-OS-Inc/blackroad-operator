  ...
  for (let shelf = 0; shelf < 3; shelf++) {
    ...
    for (let i = 0; i < 4 + shelf; i++) {
      ...
      px(
        ...
        w,
        h,
        bookColors[(shelf * 4 + i) % bookColors.length],
      )
      bx += w + 1
      if (bx > x + 8) break
    }
...
export function drawFurniture(
  ...
  switch (type) {
    case 'desk':
      return drawDesk(ctx, col, row, tick)
    case 'chair':
      return drawChair(ctx, col, row)
    case 'monitor':
      return drawMonitor(ctx, col, row, tick)
    case 'keyboard':
      return // drawn as part of desk
    case 'big_desk':
      return drawBigDesk(ctx, col, row, tick)
    case 'floor_window':
      return drawFloorWindow(ctx, col, row, tick)
    case 'coffee_machine':
      return drawCoffeeMachine(ctx, col, row, tick)
    case 'snack_shelf':
      return drawSnackShelf(ctx, col, row)
    case 'water_cooler':
      return drawWaterCooler(ctx, col, row, tick)
    case 'small_table':
      return drawSmallTable(ctx, col, row)
    case 'round_table':
      return drawRoundTable(ctx, col, row)
    case 'long_table':
      return drawLongTable(ctx, col, row)
    case 'whiteboard_obj':
      return drawWhiteboard(ctx, col, row, tick)
    case 'bookshelf':
      return drawBookshelf(ctx, col, row)
    case 'reading_chair':
      return drawReadingChair(ctx, col, row)
    case 'sofa':
      return drawSofa(ctx, col, row)
    case 'coffee_table':
      return drawCoffeeTable(ctx, col, row)
    case 'server_rack':
      return drawServerRack(ctx, col, row, tick)
    case 'potted_plant':
      return drawPottedPlant(ctx, col, row, tick)
    case 'carpet':
      return drawCarpet(ctx, col, row)
    case 'wall_clock':
      return drawWallClock(ctx, col, row, tick)
    case 'poster':
      return drawPoster(ctx, col, row)
    case 'meeting_chair':
      return drawMeetingChair(ctx, col, row)
    case 'door_mat':
      return drawDoorMat(ctx, col, row)
  }
}
