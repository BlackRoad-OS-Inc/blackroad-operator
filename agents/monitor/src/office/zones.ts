...
export function getRandomPointInZone(zone: Zone): { col: number; row: number } {
  const col =
    zone.minCol + Math.floor(Math.random() * (zone.maxCol - zone.minCol + 1))
  const row =
    zone.minRow + Math.floor(Math.random() * (zone.maxRow - zone.minRow + 1))
  return { col, row }
}
