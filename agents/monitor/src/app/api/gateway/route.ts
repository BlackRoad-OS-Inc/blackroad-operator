...
export async function GET() {
  ...
  try {
    ...
    const sessions = (result.sessions ?? []).map((s) => {
      ...
      const agentId =
        keyParts[0] === 'agent' && keyParts[1] ? keyParts[1] : 'main'
      ...
      if (agentInfo?.identity?.name ?? agentInfo?.name) {
        ...
        agentName =
          readBotName() ?? agentId.charAt(0).toUpperCase() + agentId.slice(1)
      }
    ...
    for (const sess of sessions) {
      ...
      const groupKey = sess.isSubagent
        ? sess.key
        : canonicalSessionLookupKey(sess.key)
      const existing = sessionMap.get(groupKey)
      if (
        !existing ||
        sess.totalTokens > existing.totalTokens ||
        (sess.totalTokens === existing.totalTokens &&
          (sess.lastActivity ?? 0) > (existing.lastActivity ?? 0))
      ) {
    ...
    for (const sess of dedupedSessions) {
      ...
      const parent = parentKey
        ? (byKey.get(parentKey) ??
          byKey.get(canonicalSessionLookupKey(parentKey)))
        : null
      if (parent) {
