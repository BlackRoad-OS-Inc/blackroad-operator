...
export async function POST(request: Request) {
  ...
  try {
    ...
    switch (action) {
      ...
      case 'broadcast': {
        const sessionKeys = Array.isArray(
          (body as { sessionKeys?: unknown[] }).sessionKeys,
        )
          ? ((body as { sessionKeys?: string[] }).sessionKeys ?? []).filter(
              Boolean,
            )
          : []
        if (sessionKeys.length === 0 || !message) {
          return NextResponse.json(
            { error: 'sessionKeys and message required' },
            { status: 400 },
          )
        }
        ...
        const failed = results
          ...
          .map((entry) => ({
            sessionKey: entry.sessionKey,
            error:
              (entry.result as PromiseRejectedResult).reason instanceof Error
