...
export async function POST(request: Request) {
  try {
    ...
    if (
      typeof body.defaultDirective === 'string' &&
      body.defaultDirective.trim()
    ) {
      config.defaultDirective = body.defaultDirective.trim()
    }

    if (sessionKey) {
      const updated = upsertAutoworkPolicy(
        sessionKey,
        {
          ...
          directive:
            typeof body.directive === 'string' ? body.directive : undefined,
        },
        config,
      )
      return NextResponse.json({
        ok: true,
        config: updated,
        policy: updated.policies[sessionKey],
      })
    }
