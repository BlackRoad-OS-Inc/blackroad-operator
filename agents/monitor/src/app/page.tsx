...
export default function DashboardPage() {
  ...
  const [autoworkConfig, setAutoworkConfig] =
    useState<AutoworkConfig>(DEFAULT_AUTOWORK)
  ...
  const openAgent = chatAgent
    ? agents.find((agent) => agent.id === chatAgent)
    : null
  ...
  useEffect(() => {
    ...
    if (newState.totalXP !== achievementState.totalXP) {
      ...
      setXpState((prev) =>
        addXP(prev, xpGained, 'achievements', 'Achievement unlocked!'),
      )
    }
  ...
  const saveAutoworkConfig = useCallback(
    async (patch: Partial<AutoworkConfig>) => {
      try {
  ...
  const runAutoworkNow = useCallback(
      ...
      <main className="mx-auto max-w-7xl px-4 pb-8 pt-24">
        ...
        <div className="flex gap-2 mb-6 overflow-x-auto">
          <button
            ...
            className={`px-4 py-2 rounded-lg font-mono text-sm transition-all ${activeTab === 'overview' ? 'bg-[var(--accent-primary)] text-white' : 'bg-[var(--bg-secondary)] text-[var(--text-secondary)] hover:bg-white/10'}`}
          >
            📊 Overview
          </button>
          <button
            onClick={() => setActiveTab('achievements')}
            className={`px-4 py-2 rounded-lg font-mono text-sm transition-all ${activeTab === 'achievements' ? 'bg-[var(--accent-primary)] text-white' : 'bg-[var(--bg-secondary)] text-[var(--text-secondary)] hover:bg-white/10'}`}
          >
            🏆 Achievements ({achievementState.unlockedCount}/
            {achievementState.achievements.length})
          </button>
          <button
            onClick={() => setActiveTab('leaderboard')}
            className={`px-4 py-2 rounded-lg font-mono text-sm transition-all ${activeTab === 'leaderboard' ? 'bg-[var(--accent-primary)] text-white' : 'bg-[var(--bg-secondary)] text-[var(--text-secondary)] hover:bg-white/10'}`}
          >
            🏅 Leaderboard
          </button>
          <button
            onClick={() => setActiveTab('metrics')}
            className={`px-4 py-2 rounded-lg font-mono text-sm transition-all ${activeTab === 'metrics' ? 'bg-[var(--accent-primary)] text-white' : 'bg-[var(--bg-secondary)] text-[var(--text-secondary)] hover:bg-white/10'}`}
          >
            📈 Metrics
          </button>
        </div>
