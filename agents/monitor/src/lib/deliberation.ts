...
export type DeliberationAction =
  ...
  | {
      type: 'CONCLUDE'
      outcome: 'accepted' | 'rejected' | 'withdrawn' | 'deadlocked'
    }
  ...
  switch (event.type) {
    ...
    case 'VOTE_CAST':
      return {
        ...
        metadata: {
          voteCount: session.topic.voteCount + 1,
    ...
    case 'WITHDRAW':
      return {
        nextPhase: 'concluded',
        actions: [{ type: 'CONCLUDE', outcome: 'withdrawn' }],
      }

    case 'NEXT_ROUND':
      return {
        nextPhase:
          session.roundNumber >= session.maxRounds ? 'voting' : 'arguing',
        actions: [
          {
            type: 'NOTIFY_AGENTS',
            message: `Round ${session.roundNumber + 1}/${session.maxRounds}`,
          },
        ],
...
export function recordAbstention(
  session: DeliberationSession,
  agentId: string,
): DeliberationSession {
