...
export const MODEL_PRICING: Record<string, { input: number; output: number }> =
  {
    ...
    // Bailian / Alibaba
    'qwen3.5-plus': { input: 0.0, output: 0.0 }, // Subscription
    'qwen3-max': { input: 0.0, output: 0.0 },
...
export function calculateTokenTrends(
  ...
  const inputTokens = timestamps.map((t) =>
    grouped[t].reduce((sum, d) => sum + d.input, 0),
  )
  const outputTokens = timestamps.map((t) =>
    grouped[t].reduce((sum, d) => sum + d.output, 0),
  )
  const totalTokens = timestamps.map((t) =>
    grouped[t].reduce((sum, d) => sum + d.total, 0),
  )
...
export function calculateAgentProductivity(
  ...
  const avgResponseTime =
    completedTasks.length > 0
      ? completedTasks.reduce(
          (sum, t) => sum + (t.completedAt! - t.startedAt) / 1000,
          0,
  ...
  const productivityScore =
    successRate * 0.4 + taskScore * 0.3 + tokenScore * 0.2 + responseScore * 0.1
...
export function generateDailyReport(
  ...
  const agentProductivities = Object.entries(agentStates).map(([id, state]) => {
    return calculateAgentProductivity(
      id,
      state.sessionLog[0] || 'Unknown',
      state,
      allTasks,
    )
  })
      ...
      case 'successRate':
        value =
          state.taskHistory.length > 0
            ? (state.taskHistory.filter((t) => t.status === 'completed')
                .length /
...
export function checkPerformanceAlerts(
  ...
  for (const agentId in agentStates) {
    ...
    const recentTasks = state.taskHistory.filter(
      (t) =>
        t.status === 'completed' &&
        t.completedAt &&
        t.completedAt > now - 3600000,
    )
    ...
    const recentAllTasks = state.taskHistory.filter(
      (t) => t.startedAt > now - 3600000,
    )
    if (recentAllTasks.length >= 5) {
      const failedCount = recentAllTasks.filter(
        (t) => t.status === 'failed',
      ).length
