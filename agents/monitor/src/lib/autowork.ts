...
function toPolicy(
  input: Partial<AutoworkPolicy> | undefined,
  defaultDirective: string,
): AutoworkPolicy {
  return {
    enabled: input?.enabled ?? false,
    intervalMs:
      typeof input?.intervalMs === 'number' && input.intervalMs >= 60_000
        ? input.intervalMs
        : DEFAULT_INTERVAL_MS,
    directive:
...
function parseAgentKey(
  key: string,
): { agentId: string; isSubagent: boolean } | null {
...
function buildPrompt(
  ...
  const directive =
    policy.directive.trim() || config.defaultDirective || DEFAULT_DIRECTIVE
  return [
  ...
  try {
    if (existsSync(AUTOWORK_FILE)) {
      const raw = JSON.parse(
        readFileSync(AUTOWORK_FILE, 'utf-8'),
      ) as Partial<AutoworkConfig> & {
        policies?: Record<string, Partial<AutoworkPolicy>>
      }
      const defaultDirective =
        typeof raw.defaultDirective === 'string' && raw.defaultDirective.trim()
      ...
      return {
        maxSendsPerTick:
          typeof raw.maxSendsPerTick === 'number' && raw.maxSendsPerTick >= 1
...
export async function listAutoworkTargets(): Promise<AutoworkTarget[]> {
  ...
  for (const session of result.sessions ?? []) {
    ...
    const target: AutoworkTarget = {
      ...
      sendPolicy:
        session.sendPolicy === 'allow' || session.sendPolicy === 'deny'
          ? session.sendPolicy
          : 'unknown',
...
export async function runAutoworkTick(
  forceSessionKey?: string,
): Promise<AutoworkTickResult> {
  const now = Date.now()
  ...
  const targetByKey = new Map(
    targets.map((target) => [target.sessionKey, target]),
  )
  const gw = getGatewayConnection()
  ...
  if (forceSessionKey) {
        ...
        failed: [
          {
            ...
            error: 'Session not found for autowork',
          },
        ],
      }
    }
    ...
    const policy = toPolicy(
      config.policies[forceSessionKey],
      config.defaultDirective,
    )
    try {
      await gw.request(
