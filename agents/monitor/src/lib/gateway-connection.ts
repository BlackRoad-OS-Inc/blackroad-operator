...
function resolveStateDir(): string {
  const override =
    trimValue(process.env.OPENCLAW_STATE_DIR) ??
...
function resolveConfigPath(stateDir: string): string {
  const override =
    trimValue(process.env.OPENCLAW_CONFIG_PATH) ??
...
function normalizeDevicePublicKeyBase64Url(
  publicKeyPem: string,
): string | null {
  try {
...
export function readOpenClawConfig(): GatewayConnectionConfig | null {
  ...
  const port = Number(
    envPort ??
      (typeof configGateway?.port === 'number' ||
      typeof configGateway?.port === 'string'
        ? configGateway.port
        : 18789),
  )
  ...
  const authMode = (
    envPassword
      ...
      : gatewayAuth?.mode === 'password'
        ? 'password'
        : 'token'
  ) as GatewayConnectionConfig['authMode']
...
class GatewayConnection {
  ...
  request<T = unknown>(
    ...
    return new Promise<T>((resolve, reject) => {
      ...
      if (!this.isReady) {
        ...
        if (!this.isReady) {
          this.ephemeralRequest<T>(method, params, timeoutMs).then(
            resolve,
            reject,
          )
          return
  ...
  private connect(): void {
    ...
    ws.on('open', () => {
      console.log(
        '[gateway-connection] WebSocket open, waiting for challenge...',
      )
      this.connected = true
    ...
    if (type === 'res') {
      ...
      if (pending) {
        ...
        if (msg.ok) {
          ...
          pending.reject(
            new Error(`Gateway error: ${JSON.stringify(msg.error)}`),
          )
        }
      }
      return
  ...
  private handleEvent(msg: Record<string, unknown>): void {
    ...
    if (event === 'chat') {
      if (!this.loggedFirstEvent) {
        ...
        console.log(
          '[gateway-connection] First event received:',
          event,
          'sessionKey:',
          payload.sessionKey,
          'state:',
  ...
  private createConnectParams(
    ...
    try {
      ...
      const nonce =
        typeof challenge?.nonce === 'string' ? challenge.nonce : undefined
      ...
      console.error(
        '[gateway-connection] Failed to build connect payload:',
        err,
      )
      return null
    }
  ...
  private scheduleReconnect(): void {
    ...
    const delay = Math.min(
      GatewayConnection.BASE_RECONNECT_DELAY_MS *
        ...
        try {
          ...
          if (msg.type === 'res' && msg.id === 'connect-1') {
            if (!msg.ok) {
              ...
              reject(
                new Error(
                  `Gateway connect failed: ${JSON.stringify(msg.error)}`,
                ),
              )
              return
            }
...
export function getGatewayConnection(): GatewayConnection {
  let conn = (globalThis as Record<symbol, unknown>)[GLOBAL_KEY] as
    | GatewayConnection
    | undefined
  if (!conn) {
...
export function destroyGatewayConnection(): void {
  const conn = (globalThis as Record<symbol, unknown>)[GLOBAL_KEY] as
    | GatewayConnection
    | undefined
  if (conn) {
    conn.destroy()
