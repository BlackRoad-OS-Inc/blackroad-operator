    ...
    return (
      <div className="bg-[var(--bg-card)] border border-[var(--border)] rounded-xl p-6">
        ...
        <p className="text-[var(--text-secondary)] text-sm mb-4">
          Bring all your agents together for collaborative discussions via
          OpenClaw Gateway.
        </p>
        ...
        {messages.length === 0 ? (
          ...
          <div className="space-y-3">
            {messages.map((msg) => {
              const badge =
                msg.agentType !== 'main'
                  ? getAgentTypeBadge(msg.agentType)
                  : null
              return (
                <div
                  key={msg.id}
                ...
                >
                  ...
                  <div
                    className={`flex-1 ${msg.agentId === 'user' ? 'text-right' : ''}`}
                  >
      ...
      <div className="mt-4 pt-4 border-t border-[var(--border)]">
        ...
        <div className="flex flex-wrap gap-1 max-h-24 overflow-y-auto">
          ...
          {allAgents.slice(0, 8).map((agent) => (
            <span
              key={agent.id}
              className="text-xs px-2 py-1 bg-[var(--bg-card)] rounded border"
              style={{ borderColor: agent.badge.color }}
            >
              {agent.emoji || agent.badge.emoji} {agent.name || agent.id}
            </span>
