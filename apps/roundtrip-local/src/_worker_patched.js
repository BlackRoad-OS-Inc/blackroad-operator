...
const OLLAMA_NODES = [
  { name: 'octavia',  url: 'http://localhost:11434', priority: 1, hailo: true },
];
const HAILO_NODES = [
...
async function ollamaFetch(path, body, timeoutMs = 25000) {
  for (const node of OLLAMA_NODES) {
    try {
      console.log(`[ollama] trying ${node.name}: ${node.url}${path} (timeout: ${timeoutMs}ms)`);
      const t0 = Date.now();
      const res = await fetch(node.url + path, {
        method: 'POST', headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify(body), signal: AbortSignal.timeout(timeoutMs),
      });
      console.log(`[ollama] ${node.name} responded ${res.status} in ${Date.now()-t0}ms`);
      if (res.ok) { const d = await res.json(); d._node = node.name; return d; }
    } catch (e) { console.log(`[ollama] ${node.name} failed: ${e.message}`); }
  }
  return null;
}
