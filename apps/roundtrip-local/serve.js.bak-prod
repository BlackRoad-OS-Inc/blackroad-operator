...
// SOVEREIGN: On Octavia, Ollama is localhost — no need for tunnel
workerSource = workerSource.replace(/https:\/\/ollama-internal\.blackroad\.io/g, 'http://localhost:11434')
...
async function registerNode() {
  return postJson(CLOUD_URL + '/api/autonomy/register', {
    ...
    status: 'online',
    version: VERSION,
    capabilities: ['memory', 'chat', 'collab', 'background-tasks', 'roadcode', 'slack'],
...
const server = http.createServer(async (req, res) => {
  try {
    ...
    // Ollama proxy — CF Worker calls this to reach local Ollama
    if (url.pathname.startsWith('/ollama/')) {
      const ollamaPath = url.pathname.replace('/ollama', '')
      const ollamaRes = await fetch('http://localhost:11434' + ollamaPath, {
        method: req.method,
        headers: { 'Content-Type': 'application/json' },
        body,
