...
function renderMetrics(metrics: SystemMetrics): void {
  ...
  console.log(
    p('│') +
      `  ${brand.logo()}  ${w('LIVE SYSTEM METRICS')}              ${g(metrics.timestamp.slice(11, 19))}  ` +
      p('│'),
  )
  console.log(
    p('├──────────────────────────────────────────────────────────────────┤'),
  ...
  console.log(
    p('│') +
      `  ${w('CPU')}   ${bar(metrics.cpu.usagePercent)} ${chalk.cyan(`${metrics.cpu.usagePercent}%`)}  ${g(`${metrics.cpu.count} cores`)}            ` +
      p('│'),
  )
  console.log(
    p('│') +
      `  ${w('MEM')}   ${bar(metrics.memory.usagePercent)} ${chalk.cyan(`${metrics.memory.usagePercent}%`)}  ${g(`${metrics.memory.usedMB}/${metrics.memory.totalMB}MB`)}       ` +
      p('│'),
  )
  console.log(
    p('│') +
      `  ${w('DISK')}  ${bar(metrics.disk.usagePercent)} ${chalk.cyan(`${metrics.disk.usagePercent}%`)}  ${g(`${metrics.disk.usedGB}/${metrics.disk.totalGB}GB`)}         ` +
      p('│'),
  )
  console.log(
    p('│') +
      `  ${w('LOAD')}  ${g(`1m:`)} ${chalk.cyan(String(metrics.cpu.loadAvg1m))}  ${g(`5m:`)} ${chalk.cyan(String(metrics.cpu.loadAvg5m))}  ${g(`15m:`)} ${chalk.cyan(String(metrics.cpu.loadAvg15m))}      ` +
      p('│'),
  )
  console.log(
    p('│') +
      `  ${w('NET')}   ${g('connections:')} ${chalk.cyan(String(metrics.network.openConnections))}  ${g('uptime:')} ${chalk.cyan(metrics.load.uptimeHuman)}           ` +
      p('│'),
  )

  if (metrics.memory.swapTotalMB > 0) {
    const swapPct = Math.round(
      (metrics.memory.swapUsedMB / metrics.memory.swapTotalMB) * 100,
    )
    console.log(
      p('│') +
        `  ${w('SWAP')}  ${bar(swapPct)} ${chalk.cyan(`${swapPct}%`)}  ${g(`${metrics.memory.swapUsedMB}/${metrics.memory.swapTotalMB}MB`)}      ` +
        p('│'),
    )
  }
  ...
  console.log(
    p('│') +
      `  ${w('PROC')}  ${g('total:')} ${chalk.cyan(String(metrics.process.total))}  ${g('node:')} ${chalk.cyan(String(metrics.process.nodeProcesses))}  ${g('python:')} ${chalk.cyan(String(metrics.process.pythonProcesses))}       ` +
      p('│'),
  )
  console.log(
    p('└──────────────────────────────────────────────────────────────────┘'),
...
function renderServices(probes: ServiceProbe[]): void {
  ...
  console.log(
    p('│') +
      `  ${chalk.bold.white('SERVICE HEALTH')}                                                ` +
      p('│'),
  )
  console.log(
    p('├──────────────────────────────────────────────────────────────────┤'),
...
function renderBottleneck(b: Bottleneck): void {
  ...
  console.log(
    `              ${chalk.gray('metric:')} ${chalk.cyan(b.metric)}  ${chalk.gray('threshold:')} ${chalk.yellow(b.threshold)}`,
  )
...
function renderReport(report: BottleneckReport): void {
  ...
  console.log(
    p('│') +
      `  ${chalk.bold.white('BOTTLENECK ANALYSIS')}          Score: ${scoreColor(report.score)}/100                ` +
      p('│'),
  )
  console.log(
    p('├──────────────────────────────────────────────────────────────────┤'),
  ...
  if (report.bottlenecks.length === 0) {
    console.log(
      p('│') +
        `  ${chalk.green('✓ No bottlenecks detected. All systems nominal.')}                ` +
        p('│'),
    )
  }
...
export const bottlenecksCommand = new Command('bottlenecks')
  ...
  .action(
    async (opts: { json?: boolean; watch?: boolean; interval?: string }) => {
      const run = async () => {
      ...
      if (opts.watch) {
        ...
        logger.info(
          `Watching bottlenecks every ${intervalSec}s. Press Ctrl+C to stop.`,
        )
        console.log()
