...
function CountUp({ end = 67, label = "agents" }) {
...
const ALL_ANIMS = [
  ...
  { title:"Count Up",           tag:"Text",     cat:"Text",      accent:"#8844FF", el:<CountUp end={67} label="agents" />, code:`// IntersectionObserver + requestAnimationFrame\nuseEffect(() => {\n  let frame, start;\n  const tick = (ts) => {\n    if(!start) start = ts;\n    const p = Math.min((ts-start)/1800, 1);\n    const ease = 1 - Math.pow(1-p, 4);\n    setValue(Math.floor(ease * target));\n    if(p<1) frame = requestAnimationFrame(tick);\n  };\n  frame = requestAnimationFrame(tick);\n  return () => cancelAnimationFrame(frame);\n}, [started]);` },
