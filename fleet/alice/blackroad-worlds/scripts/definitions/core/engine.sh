...
remember() {
    ...
    sqlite3 "$db" "INSERT INTO memories (role, content) VALUES ('$role', '$(echo "$content" | sed "s/'/''/g")');"
}
...
learn_fact() {
    ...
    sqlite3 "$db" "INSERT OR REPLACE INTO facts (key, value) VALUES ('$key', '$(echo "$value" | sed "s/'/''/g")');"
}
...
infer_ollama() {
    ...
    # Use Ollama API for system prompt support
