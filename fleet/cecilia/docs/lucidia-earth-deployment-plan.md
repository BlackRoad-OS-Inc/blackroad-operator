...
- **aria64 (192.168.4.64)**: Main web server hosting all BlackRoad sites
  - Running BlackRoad OS reverse proxy (ports 80/443)
  - 20+ Next.js containers on ports 3000-3031
  - Portainer for container management
- **lucidia (192.168.4.38)**: Available Pi (needs Docker permissions)
- **shellfish (174.138.44.45)**: DigitalOcean droplet with BlackRoad OS + LLM service
...
- BlackRoad OS reverse proxy configured and working
...
### BlackRoad OS Configuration Addition
```blackroad osfile
...
### 3. Update BlackRoad OS
```bash
# SSH to aria64
ssh aria64

# Backup current config
docker exec blackroad os cat /etc/blackroad os/BlackRoad OSfile > ~/BlackRoad OSfile.backup

# Add lucidia.earth config
docker exec blackroad os sh -c 'cat >> /etc/blackroad os/BlackRoad OSfile << EOF
...
### BlackRoad OS Status
```bash
ssh aria64 "docker logs blackroad os | tail -50"
```
...
ssh aria64 "docker exec blackroad os cat /etc/blackroad os/BlackRoad OSfile > ~/BlackRoad OSfile.$(date +%Y%m%d)"
```
...
### If BlackRoad OS config breaks:
```bash
# Restore backup
ssh aria64 "cat ~/BlackRoad OSfile.backup | docker exec -i blackroad os tee /etc/blackroad os/BlackRoad OSfile"
...
- **BlackRoad OS**: Handles automatic HTTPS via Let's Encrypt
...
- **Static Asset Caching**: BlackRoad OS caches static files
...
- Use BlackRoad OS load balancing or Cloudflare health checks
