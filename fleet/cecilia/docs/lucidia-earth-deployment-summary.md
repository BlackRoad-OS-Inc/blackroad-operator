...
- **Reverse Proxy:** BlackRoad OS (automatic HTTPS)
...
| **BlackRoad OS Proxy** | aria64 | Routes lucidia.earth → localhost:3040 |
...
- **Via BlackRoad OS (when DNS points):** https://lucidia.earth
...
Added to `/etc/blackroad os/BlackRoad OSfile` in BlackRoad OS container:
...
To reload BlackRoad OS after changes:
...
aria64 (192.168.4.64)
    ↓
BlackRoad OS :80/443 (auto HTTPS)
    ↓
...
5. **Proxy Strategy**: BlackRoad OS handles HTTPS, Cloudflare adds security
...
- [x] Configure BlackRoad OS reverse proxy
