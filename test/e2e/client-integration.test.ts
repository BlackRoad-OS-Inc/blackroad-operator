...
function handler(req: IncomingMessage, res: ServerResponse) {
  ...
  if (req.url === '/v1/health' && req.method === 'GET') {
    res.end(
      JSON.stringify({ status: 'healthy', version: '2.0.0', uptime: 12345 }),
    )
    return
  }
  ...
  if (req.url === '/v1/agents' && req.method === 'GET') {
    res.end(
      JSON.stringify({
        agents: [{ name: 'cipher', title: 'The Hacker', role: 'security' }],
      }),
    )
    return
  }
  ...
  if (req.url === '/v1/invoke' && req.method === 'POST') {
    ...
    req.on('end', () => {
      ...
      res.end(
        JSON.stringify({ content: `${parsed.agent}: ${parsed.task} done` }),
      )
    })
    return
  }
...
describe('GatewayClient integration e2e', () => {
  ...
  it('should GET /v1/agents from real HTTP server', async () => {
    const client = new GatewayClient(`http://127.0.0.1:${port}`)
    const result = await client.get<{ agents: Array<{ name: string }> }>(
      '/v1/agents',
    )
  ...
  it('should throw on error status code from real server', async () => {
    const client = new GatewayClient(`http://127.0.0.1:${port}`)
    await expect(client.get('/v1/error')).rejects.toThrow(
      'GET /v1/error failed: 503',
    )
  })

  it('should throw on 404 from real server', async () => {
    const client = new GatewayClient(`http://127.0.0.1:${port}`)
    await expect(client.get('/v1/nonexistent')).rejects.toThrow(
      'GET /v1/nonexistent failed: 404',
    )
  })
