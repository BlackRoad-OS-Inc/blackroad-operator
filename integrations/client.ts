...
// ─── Auth Resolver ──────────────────────────────────────────────────
function resolveAuth(config: IntegrationConfig): Record<string, string> {
...
export class BlackRoadIntegrations {
  ...
  async request<T = unknown>(
    ...
    if (useGateway) {
      const gatewayRoute = (config as unknown as Record<string, string>)
        .gateway_route
  ...
  async healthCheck(): Promise<HealthResult[]> {
    ...
    for (const config of this.manifest.integrations) {
      ...
      if (['local', 'ssh_key'].includes(config.auth_type) && !config.api_base) {
        results.push({
          id: config.id,
          name: config.name,
          status: 'healthy',
  ...
  /** Linear: Create issue */
  async linearCreateIssue(title: string, teamId: string, description?: string) {
