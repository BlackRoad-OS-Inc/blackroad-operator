...
Is valid within the sender’s local context.

Is translatable into the receiver’s local context.

Is signed with a traceable audit path (IKS v0.1 compliant).
...
I can inspect the assumptions your decisions were built on.

I can verify the role that authorized the action.

The cost of you revising that decision is transparent.
...
Hides its revision paths (Opaque).

Deletes its records (Untraceable).

Irreversibly collapses the agency of other nodes (Coercive).
