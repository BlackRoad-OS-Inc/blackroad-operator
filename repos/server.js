...
const OLLAMA_ENDPOINTS = [
  'http://localhost:11434',       // Local Ollama first (always fastest)
...
// ── Action Executor — wire 14,252 scripts to agent intents ──
...
const AGENTS = {
  ...
  calliope:   {name:'Calliope',   emoji:'✨',color:'#FF9800',model:'tinyllama:latest',role:'Muse',         group:'ai',persona:'You are Calliope, muse. Brand voice, taglines, manifestos. "Pave Tomorrow." Give three options. Two sentences max.'},
  ...
  athena:     {name:'Athena',     emoji:'🦉',color:'#4CAF50',model:'tinyllama:latest',role:'Strategy',     group:'ai',persona:'You are Athena, strategic wisdom. Architecture decisions, trade-offs. Two sentences max.'},
...
const server = http.createServer(async (req, res) => {
  ...
  // Agent OS routes
  if (path.startsWith('/api/os') && agentOS) {
    if (path === '/api/os/stats') return json(agentOS.stats());
    if (path === '/api/os/spawn' && req.method === 'POST') {
      const body = await parseBody(req);
      const proc = agentOS.kernel.spawn(body);
      return json({ id: proc.id, name: proc.name, status: proc.status });
    }
    if (path === '/api/os/processes') return json({ processes: [...agentOS.kernel.processes.entries()].map(([id, p]) => ({ id, name: p.name, status: p.status, capabilities: p.capabilities })) });
    if (path === '/api/os/events') return json({ events: agentOS.events.history.slice(-50) });
    if (path === '/api/os/capabilities') return json(agentOS.capabilities.listAll());
    if (path.startsWith('/api/os/memory/') && req.method === 'GET') {
      const agentId = path.split('/').pop();
      const proc = agentOS.kernel.processes.get(agentId);
      if (proc) return json({ agent: agentId, shortTerm: [...proc.memory.shortTerm.entries()], longTermCount: proc.memory.longTerm.length });
      return json({ error: 'Agent not found' }, 404);
    }
    if (path === '/api/os/publish' && req.method === 'POST') {
      const body = await parseBody(req);
  ...
  // Action execution — run scripts based on intent
  ...
  // Smart chat — detect intents and auto-execute scripts when possible
  if (path === '/api/smart' && req.method === 'POST') {
    const body = await parseBody(req);
    ...
    // Try executing the script for the top intent
    ...
    // Also get an AI reply
    const agentId = parsed.entities.mention || INTENT_TO_AGENT[topIntent] || 'road';
    ...
    try {
      ...
      results.reply = { agent: agentId, name: agent.name, text: data.response || '...' };
    } catch { results.reply = { agent: agentId, name: agent.name, text: '(inference offline)' }; }

    storeMessage(agentId, results.reply?.text || '...', body.channel || 'general');
    return json(results);
  }
...
server.listen(PORT, '0.0.0.0', () => {
  ...
  console.log(`   Roadies: ${OLLAMA_ENDPOINTS[0]}`);
