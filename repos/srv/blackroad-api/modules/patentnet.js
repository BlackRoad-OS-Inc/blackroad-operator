...
module.exports = function attachPatentNet({ app }) {
  ...
  app.post('/api/patent/claim', async (req, res) => {
    try {
      ...
      const norm = Buffer.from(
        JSON.stringify(env, Object.keys(env).sort()),
        'utf8'
      );
      const hash = sha256(norm);
      const day = todayInt();
      const base = `${day}-${hash.slice(2, 10)}-${env.title
        .toLowerCase()
        .replace(/[^a-z0-9]+/g, '-')
        .slice(0, 48)}`;
      ...
      if (Array.isArray(b.attachments)) {
        for (const a of b.attachments) {
          ...
          const p = path.join(
            dir,
            `${base}-${a.name.replace(/[^a-zA-Z0-9_.-]+/g, '_')}`
          );
      ...
      if (contract) {
        const tx = await contract.registerClaim(
          ...
          hash,
          uri,
          String(b.claimType || 'defensive-publication')
        );
  ...
  app.get('/api/patent/list', (req, res) => {
    ...
    OK(
      res,
      files.map((p) => ({ path: p, mtime: fs.statSync(p).mtimeMs }))
    );
  });
    ...
    try {
      ...
      function merkle(arr) {
        ...
        for (let i = 0; i < arr.length; i += 2) {
          const a = arr[i],
            b = arr[i + 1] || arr[i]; // duplicate last if odd
          next.push(
            crypto
              .createHash('sha256')
