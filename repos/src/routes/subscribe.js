...
router.get("/subscribe/feature-gates", requireAuth, (req, res) => {
  const sub = db.prepare('SELECT plan_id, status FROM subscriptions WHERE user_id = ?')
    .get(req.session.userId);
...
function _webhookHandlerDisabled(_req, res) {
...
module.exports = { router, webhookHandler: function(req,res){res.status(400).send("stripe_disabled")} };
