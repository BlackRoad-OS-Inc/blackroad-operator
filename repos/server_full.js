...
let subscribe; try { subscribe = require('./src/routes/subscribe'); } catch(e) { console.warn('[WARN] subscribe:', e.message); subscribe = { router: require('express').Router(), webhookHandler: (req,res) => res.json({ok:true}) }; }
...
const db = require('./src/db-compat');
...
// ── Live Fleet Data (from stats-blackroad worker) ──
...
app.get('/api/fleet', async (req, res) => {
  try {
    const now = Date.now();
    if (fleetCache && now - fleetCacheTime < 30000) {
      return res.json(fleetCache);
    }
    const resp = await fetch(STATS_URL + '/fleet');
    const data = await resp.json();
    fleetCache = data.data || data;
    fleetCacheTime = now;
    res.json(fleetCache);
  } catch (e) {
    res.json({ error: e.message, nodes: [] });
  }
});
...
app.get('/api/fleet/node/:name', async (req, res) => {
  try {
    const resp = await fetch(STATS_URL + '/fleet');
    const data = await resp.json();
    const nodes = (data.data || data).nodes || [];
    const node = nodes.find(n => n.name.toLowerCase() === req.params.name.toLowerCase());
...
app.get('/api/health', async (req, res) => {
  try {
    ...
    const online = nodes.filter(n => n.status === 'online').length;
    res.json({
      service: 'prism-enterprise',
      status: 'operational',
      version: '1.0.0',
      fleet: { online, total: nodes.length },
      uptime: process.uptime(),
      timestamp: new Date().toISOString(),
    });
  } catch (e) {
    res.json({ service: 'prism-enterprise', status: 'operational', uptime: process.uptime() });
  }
});
...
app.get('/api/kpis', async (req, res) => {
  try {
    ...
    const nodes = (data.data || data).nodes || [];
    const online = nodes.filter(n => n.status === 'online').length;
    const totalModels = nodes.reduce((sum, n) => sum + (n.ollama_models || 0), 0);
    const totalContainers = nodes.reduce((sum, n) => sum + (n.docker_containers || 0), 0);
    const totalPorts = nodes.reduce((sum, n) => sum + (n.tcp_ports || 0), 0);
    res.json({
      fleet: `${online}/${nodes.length}`,
      models: totalModels,
...
// Routes
try { const apiRouter = require('./src/routes'); app.use('/api', apiRouter); } catch(e) { console.warn('[WARN] API routes:', e.message); }

// MCI routes
try { const mciRouter = require('./srv/blackroad-api/mci/routes/mci.routes'); app.use('/api/mci', mciRouter); } catch(e) { console.warn('[WARN] MCI:', e.message); }
// Lucidia Brain routes
if (process.env.ENABLE_LUCIDIA_BRAIN !== '0') {
  try { app.use('/api/lucidia/brain', require('./routes/lucidia-brain')); } catch(e) { console.warn('[WARN] Lucidia brain:', e.message); }
}

// HTTP server + Socket.IO
const server = http.createServer(app);
let io; try { const { setupSockets } = require('./src/socket'); io = setupSockets(server); } catch(e) { console.warn('[WARN] Socket.IO:', e.message); }

// LUCIDIA
try { (function setupLucidia() {
  // --- LLM Provider
  ...
  lucidiaRouter.post('/conversations/:id/message', async (req, res) => {
    ...
    if (io) io.to(`lucidia:${convo.project_id}`).emit('lucidia:message:new', {
...
})(); } catch(e) { console.warn('[WARN] Lucidia:', e.message); }

// LANDING
// Minimal landing page + stats + admin CMS endpoints
try { (function setupLanding() {
  ...
  function refreshStats() {
    ...
    if (io) io.emit('public:stats', stats);
    return stats;
  }
...
} catch(e) { console.warn('[WARN] Landing:', e.message); }
server.listen(PORT, () => {
