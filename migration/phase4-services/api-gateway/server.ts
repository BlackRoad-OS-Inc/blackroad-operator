...
app.get('/api/status', async (c) => {
  ...
  try {
    ...
    checks.kv_cache = 'healthy'
  } catch {
    checks.kv_cache = 'down'
  }
...
app.get('/api/analytics', async (c) => {
  try {
    const tot = await DB.prepare('SELECT COUNT(*) as c FROM analytics').first<{
      c: number
    }>()
    const hr = await DB.prepare(
      'SELECT COUNT(*) as c FROM analytics WHERE ts > ?',
    )
...
app.get('/api/subdomains', (c) =>
  c.json({ subdomains: SUBDOMAINS, count: SUBDOMAINS.length }),
)
