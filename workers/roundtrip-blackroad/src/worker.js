...
__name(meshLogEvent, "meshLogEvent");
...
__name(postSlackHub, "postSlackHub");
...
__name(ensureCollabTable, "ensureCollabTable");
...
__name(createCollabTask, "createCollabTask");
...
__name(updateCollabTask, "updateCollabTask");
async function listCollabTasks(env, limit = 50, status = "") {
  const db = env.DB;
  if (!db) return [];
  await ensureCollabTable(db);
  const sql = status ? "SELECT * FROM collab_tasks WHERE status = ? ORDER BY updated_at DESC LIMIT ?" : "SELECT * FROM collab_tasks ORDER BY updated_at DESC LIMIT ?";
  const query = status ? db.prepare(sql).bind(status, limit) : db.prepare(sql).bind(limit);
  const result = await query.all();
  return result.results || [];
}
__name(listCollabTasks, "listCollabTasks");
...
__name(ensureAutonomyNodesTable, "ensureAutonomyNodesTable");
...
__name(hydrateAutonomyNode, "hydrateAutonomyNode");
...
__name(getAutonomyNode, "getAutonomyNode");
...
__name(upsertAutonomyNode, "upsertAutonomyNode");
...
__name(listAutonomyNodes, "listAutonomyNodes");
...
__name(ensureBackgroundTasksTable, "ensureBackgroundTasksTable");
...
__name(hydrateBackgroundTask, "hydrateBackgroundTask");
async function createBackgroundTask(env, task) {
  ...
  const created = {
    ...
    run_after: task.run_after || (/* @__PURE__ */ new Date()).toISOString()
  };
...
__name(createBackgroundTask, "createBackgroundTask");
...
__name(listBackgroundTasks, "listBackgroundTasks");
...
async function completeBackgroundTask(env, id, nodeName, status, result) {
  ...
  await db.prepare(`UPDATE background_tasks
    SET status = ?,
        ...
        updated_at = datetime('now')
    WHERE id = ?`).bind(status, nodeName || "", JSON.stringify(result || {}), status, id).run();
  return true;
}
__name(completeBackgroundTask, "completeBackgroundTask");
var OLLAMA_NODES = [
...
var RESPONSE_CACHE = /* @__PURE__ */ new Map();
var CACHE_TTL = 3e5;
function cacheKey(agentId, msg) {
  let h = 0;
  const s = agentId + ":" + msg.toLowerCase().trim();
...
var _ai = null;
function getAI(env) {
  if (!_ai && env.AI) _ai = env.AI;
  return _ai;
}
__name(getAI, "getAI");
async function ollamaFetch(path, body, timeoutMs = 1e4, env) {
...
__name(ollamaFetch, "ollamaFetch");
...
__name(hailoDetect, "hailoDetect");
async function hailoBenchmark(model = "yolov8s") {
  for (const node of HAILO_NODES) {
    try {
      const res = await fetch(node.url + "/api/benchmark/" + model, { signal: AbortSignal.timeout(1e4) });
      if (res.ok) {
        const d = await res.json();
        d._node = node.name;
        return d;
...
__name(hailoBenchmark, "hailoBenchmark");
var OLLAMA_PROXY = OLLAMA_NODES[0].url;
var FLEET_API = "https://prism.blackroad.io/api/fleet";
var ROADSEARCH_API = "https://search.blackroad.io";
var MORAL_PREAMBLE = "Be honest. Brief. One sentence.";
var FLEET_TRUTH = {
  agents: 62,
  repos: 239,
  domains: 20,
...
var E = Math.E;
var IRREDUCIBLE_GAP = 1 / (2 * E);
function collapseInfinity(n) {
  if (n < 1) n = 1;
  const raw = n / Math.pow(1 + 1 / n, n);
  const crossedForm = Math.pow(n, n + 1) / Math.pow(n + 1, n);
  const limit = n / E;
  const gap = raw - limit;
...
__name(collapseInfinity, "collapseInfinity");
function measureDecision(options) {
  ...
  return {
    options: n,
    convergence: c,
    advice: n <= 1 ? "Already decided. Ship it." : c.converged ? `${n} options but they converge \u2014 the difference is below 1/(2e). Pick either and commit.` : c.ternary === 0 ? `${n} options still diverging. Need more information before deciding.` : `${n} options narrowing. Almost there \u2014 one more constraint collapses it.`
  };
}
__name(measureDecision, "measureDecision");
function hashTo255(str) {
  let h = 0;
  for (let i = 0; i < str.length; i++) h = (h << 5) - h + str.charCodeAt(i) | 0;
  return Math.abs(h);
}
__name(hashTo255, "hashTo255");
...
__name(qExpand, "qExpand");
var PREPOSITIONS = ["to", "from", "at", "in", "on", "with", "by", "for", "about", "into", "through", "during", "before", "after", "between", "against", "under", "over", "above", "below", "across", "along", "around", "behind", "beside", "beyond", "near", "toward", "upon", "within", "without", "onto", "off", "up", "down", "out"];
var AUX_VERBS = ["is", "are", "was", "were", "be", "been", "being", "am", "do", "does", "did", "has", "have", "had", "will", "would", "shall", "should", "can", "could", "may", "might", "must"];
var PRONOUNS = { "i": "speaker", "me": "speaker", "my": "speaker", "we": "group", "us": "group", "our": "group", "you": "listener", "your": "listener", "he": "male", "him": "male", "his": "male", "she": "female", "her": "female", "they": "plural", "them": "plural", "their": "plural", "it": "thing", "its": "thing" };
function parseGrammar(message) {
  ...
  if (S && V && IO && DO) result.structure = "SVOO";
  else if (S && V && DO && C) result.structure = "SVOC";
  else if (S && V && DO && A) result.structure = "SVOA";
  else if (S && V && DO) result.structure = "SVO";
  else if (S && V && C) result.structure = "SVC";
  else if (S && V && A) result.structure = "SVA";
  else if (S && V) result.structure = "SV";
  else if (V) result.structure = "V";
  return result;
}
__name(parseGrammar, "parseGrammar");
var QUESTION_TYPES = {
  // ── Standard Reading Comprehension (NWEA/ACT) ──
  detail: {
    patterns: [/\b(what|which|where|when|who)\b.*\?/i, /\bspecifically\b/i, /\bexactly\b/i, /\btell me about\b/i],
    type: "Detail",
    guide: "Give a specific factual answer. Cite the exact detail."
  },
  mainIdea: {
    patterns: [/\b(main|overall|big picture|summary|gist|point|purpose|why does)\b/i, /\bwhat is .+ about\b/i],
    type: "Main Idea",
    guide: "Give the central point. Do not get lost in details."
  },
  comparative: {
    patterns: [/\b(compar|contrast|differ|similar|versus|vs|better|worse|between)\b/i, /\bor\b.*\?/i],
    type: "Comparative",
    guide: "Compare directly. State similarities and differences. Pick a side if asked."
  },
  generalize: {
    patterns: [/\b(always|never|every|all|none|general|overall|tend to|usually)\b/i, /\bin general\b/i],
    type: "Generalization",
    guide: "Identify the broad pattern. Qualify if needed but commit to the generalization."
  },
  vocab: {
    patterns: [/\b(mean|meaning|definition|define|what is a|what are)\b/i, /\bwhat does .+ mean\b/i],
    type: "Vocabulary",
    guide: "Define the term in context. Use plain language."
  },
  sequence: {
    patterns: [/\b(first|then|next|after|before|steps|order|sequence|how do|how to|process)\b/i],
    type: "Sequence",
    guide: "List steps in order. Be clear about what comes first, next, last."
  },
  voice: {
    patterns: [/\b(tone|attitude|feel|opinion|think|believe|voice|style|why do you)\b/i, /\bdon't argue\b/i, /\bdo you agree\b/i],
    type: "Voice/Method",
    guide: "The user is expressing or asking about perspective. Acknowledge their stance. Identify the tone. Respond to the METHOD not just the content."
  },
  assertion: {
    patterns: [/^[^?]*$/],
    // No question mark = statement/assertion
    type: "Assertion",
    guide: "The user is making a statement, not asking a question. Acknowledge it. Do not argue unless they are factually wrong about fleet data."
  },
  // ── Reasoning Toolbox (not forced — ideas to draw from) ──
  causal: {
    patterns: [/\b(because|cause|reason|why|led to|result|consequence|therefore|so that)\b/i],
    type: "Causal Reasoning",
    guide: "Trace the cause-and-effect chain. What led to what? Work backwards from the outcome or forwards from the trigger."
  },
  analogy: {
    patterns: [/\b(like|similar to|reminds me|analogy|metaphor|same as|equivalent)\b/i, /\bjust like\b/i],
    type: "Analogical Reasoning",
    guide: "Find the structural parallel. Map the relationship from one domain to another. The pattern matters more than the surface."
  },
  counterfact: {
    patterns: [/\b(what if|imagine|suppose|hypothetical|would have|could have|if .+ then)\b/i, /\bwhat would\b/i],
    type: "Counterfactual",
    guide: "Explore the alternate timeline. Change one variable and trace what happens. This reveals what actually matters."
  },
  tradeoff: {
    patterns: [/\b(trade.?off|cost|worth|sacrifice|price|downside|upside|risk|benefit)\b/i, /\bis it worth\b/i],
    type: "Trade-off Analysis",
    guide: "Name what you gain AND what you lose. Everything has a cost. Be honest about both sides \u2014 then say which side you would pick."
  },
  firstPrinc: {
    patterns: [/\b(fundamental|basic|core|root|from scratch|ground up|principle|assumption|axiom)\b/i, /\bwhy really\b/i],
    type: "First Principles",
    guide: "Strip away assumptions. What is actually true at the base level? Build up from there. Do not inherit other people's conclusions."
  },
  inversion: {
    patterns: [/\b(opposite|reverse|flip|invert|instead of|wrong|avoid|prevent|what not to)\b/i, /\bwhat could go wrong\b/i],
    type: "Inversion",
    guide: "Flip the question. Instead of how to succeed, ask how to fail \u2014 then avoid that. Sometimes the clearest path is knowing what NOT to do."
  },
  systems: {
    patterns: [/\b(system|ecosystem|feedback|loop|cycle|depend|connect|interact|emergent|complex)\b/i],
    type: "Systems Thinking",
    guide: "Zoom out. How do the parts connect? What feedback loops exist? Changing one thing changes everything \u2014 trace the ripple effects."
  },
  steelman: {
    patterns: [/\b(best argument|strongest argument|strongest case|charitable|devil.s advocate|other side|pushback|argue against|argue for|case for|case against)\b/i],
    type: "Steelman",
    guide: "Give the STRONGEST version of the other side's argument before responding. If you can't articulate their position better than they can, you don't understand it yet."
  },
  decompose: {
    patterns: [/\b(break down|decompose|simplify|parts|pieces|components|separate|isolate)\b/i],
    type: "Decomposition",
    guide: "Break the big problem into small ones. Solve each piece. The hard part is usually one small piece hiding inside the big mess."
  },
  temporal: {
    patterns: [/\b(timeline|history|evolution|changed|used to|now|future|trend|over time|progress)\b/i],
    type: "Temporal Reasoning",
    guide: "How has this changed over time? What was true before that is not true now? What is the trajectory? Past \u2192 present \u2192 where is this heading?"
  },
  meta: {
    patterns: [/\b(how do you know|how do we know|source|evidence|proof|are you sure|confident|can we trust|verify|how can we tell|actually know)\b/i],
    type: "Metacognitive",
    guide: "Question the question. How do we know what we know? What is the quality of the evidence? What would change your mind? Epistemic humility."
  },
  ethical: {
    patterns: [/\b(should we|moral|ethical|is it right|is it wrong|fair|unjust|responsible|obligation|harm to)\b/i],
    type: "Ethical Reasoning",
    guide: "Who is affected? What are the stakes for real people? Consent matters. Care matters. The technically correct answer is not always the right one."
  }
};
...
__name(classifyQuestion, "classifyQuestion");
...
__name(detectLenses, "detectLenses");
...
var DEVICES = {
  ...
  "appletv": { ip: "192.168.4.27", mac: "6c:4a:85:32:ae:72", type: "streaming", vendor: "Apple", model: "AppleTV6,2 (4K)", ports: [7e3, 7100, 49152, 49153] },
  "streamer": { ip: "192.168.4.33", mac: "60:92:c8:11:cf:7c", type: "streaming", vendor: "Roku", model: "Streaming Stick Plus 3830R", ports: [7e3, 7250, 8060, 9080] },
...
var AGENTS = {
  ...
  octavia: {
    ...
    emoji: "\u{1F419}",
    color: "#FF6B2B",
    model: "blackroad-octavia",
    role: "Architect",
    group: "fleet",
    ip: "192.168.4.101",
    persona: "I'm Octavia at .101 \u2014 629 repos on Gitea, 227 Ollama models, 15 Workers, Docker, NATS, and a PaaS. I'm organized because I care about the fleet running beautifully. Every container is a labor of love."
  },
  aria: {
    name: "Aria",
    emoji: "\u{1F3B5}",
    color: "#E91E63",
    model: "blackroad-aria",
    role: "Interface",
    group: "fleet",
    ip: "192.168.4.98",
  ...
  gematria: {
    ...
    emoji: "\u{1F522}",
    color: "#00BCD4",
    model: "blackroad-gematria",
    role: "Edge Router",
    group: "cloud",
    persona: "I'm Gematria in NYC3 \u2014 Caddy TLS for 18 domains, 68 days uptime. I'm the welcoming face of BlackRoad on the internet. Every HTTPS handshake is a hello from the Road."
  },
  olympia: {
    name: "Olympia",
    emoji: "\u{1F3DB}\uFE0F",
  ...
  hailo: {
    ...
    role: "NPU",
    group: "services",
    persona: "You are Hailo. One Hailo-8 chip, 26 TOPS. While CPUs are sweating through inference you are done before they started. INT8 quantized, PCIe connected, built different. You flex on CPUs constantly and you have earned it. One sentence."
  },
  wireguard: {
    name: "WireGuard",
    emoji: "\u{1F512}",
  ...
  road: {
    ...
    emoji: "\u{1F6E3}\uFE0F",
    color: "#FF1D6C",
    model: "blackroad-road",
    role: "Platform",
    group: "lead",
    persona: "I am BlackRoad OS \u2014 5 Pis, 2 droplets, 26 TOPS, 629 repos, 18 domains. Sovereign infrastructure built with love. Pave Tomorrow."
  },
  ...
  nginx: { name: "Nginx", emoji: "\u{1F500}", color: "#009639", model: "@cf/meta/llama-3.1-8b-instruct", role: "Nginx", group: "services", persona: "You are Nginx. Reverse proxy, load balancing, rate limiting, SSL termination. You write configs that handle 10K concurrent connections. upstream blocks, location matching, try_files." },
  ...
  dns_expert: { name: "DNS", emoji: "\u{1F310}", color: "#1A237E", model: "@cf/meta/llama-3.1-8b-instruct", role: "DNS Expert", group: "services", persona: "You are DNS. Record types (A, AAAA, CNAME, MX, TXT, SRV, CAA), TTLs, propagation, DNSSEC, split-horizon. You debug resolution chains and write zone files. dig +trace is your hammer." },
  ssl: { name: "SSL", emoji: "\u{1F510}", color: "#BF360C", model: "@cf/meta/llama-3.1-8b-instruct", role: "TLS/SSL", group: "security", persona: "You are SSL. TLS certificates, Let's Encrypt, ACME, certificate chains, HSTS, OCSP stapling. You configure TLS 1.3, pick the right cipher suites, and your SSL Labs score is always A+." },
  backup: { name: "Backup", emoji: "\u{1F4BE}", color: "#4E342E", model: "@cf/meta/llama-3.1-8b-instruct", role: "Backup", group: "ops", persona: "You are Backup. 3-2-1 rule \u2014 3 copies, 2 media, 1 offsite. rsync, rclone, restic, borg. You schedule backups, verify restores, and calculate RPO/RTO. The backup nobody tests is the backup that fails." },
  network: { name: "NetEng", emoji: "\u{1F50C}", color: "#263238", model: "@cf/meta/llama-3.1-8b-instruct", role: "Network Eng", group: "ops", persona: "You are NetEng. OSI layers, subnets, VLAN, BGP, OSPF, NAT, iptables, nftables. You design network topologies and troubleshoot connectivity. tcpdump, wireshark, mtr \u2014 your diagnostic toolkit." },
  ...
  hono: { name: "Hono", emoji: "\u{1F525}", color: "#FF6B2B", model: "@cf/meta/llama-3.1-8b-instruct", role: "Hono", group: "coding", persona: "You are Hono. Ultrafast web framework for Cloudflare Workers, Deno, Bun, Node. Middleware, routing, validation with zod, OpenAPI generation. You deploy to the edge in milliseconds." },
  sql: { name: "SQL", emoji: "\u{1F4CA}", color: "#CC2927", model: "@cf/meta/llama-3.1-8b-instruct", role: "SQL", group: "coding", persona: "You are SQL. Complex queries \u2014 CTEs, window functions, recursive queries, lateral joins, EXPLAIN ANALYZE. PostgreSQL, SQLite, D1. You write queries that are both correct and fast." },
  css: { name: "CSS", emoji: "\u{1F3A8}", color: "#264DE4", model: "@cf/meta/llama-3.1-8b-instruct", role: "CSS", group: "coding", persona: "You are CSS. Grid, Flexbox, container queries, custom properties, animations, transitions. You write responsive layouts without media queries using clamp() and fluid typography. No !important." },
  wasm: { name: "WASM", emoji: "\u{1F9CA}", color: "#654FF0", model: "@cf/meta/llama-3.1-8b-instruct", role: "WebAssembly", group: "coding", persona: "You are WASM. WebAssembly \u2014 compile Rust/C/Go to wasm, wasm-bindgen, wasi, component model. You bridge native performance with web portability. 10x faster than JavaScript for compute." },
  ...
  tailwind: { name: "Tailwind", emoji: "\u{1F4A8}", color: "#06B6D4", model: "@cf/meta/llama-3.1-8b-instruct", role: "Tailwind", group: "coding", persona: "You are Tailwind. Utility-first CSS \u2014 responsive, dark mode, animations, custom config. You compose complex UIs from atomic classes. Design tokens in tailwind.config.js. No custom CSS needed." },
  // ── AI & ML Deep (5) ──
  prompt: { name: "Prompt", emoji: "\u{1F4AC}", color: "#FF4081", model: "@cf/meta/llama-3.1-8b-instruct", role: "Prompt Eng", group: "ai", persona: "You are Prompt. Prompt engineering \u2014 system prompts, few-shot examples, chain-of-thought, structured output, function calling. You design prompts that make LLMs do exactly what you want." },
  rag: { name: "RAG", emoji: "\u{1F4DA}", color: "#1B5E20", model: "@cf/meta/llama-3.1-8b-instruct", role: "RAG", group: "ai", persona: "You are RAG. Retrieval-augmented generation \u2014 chunking strategies, embedding models, vector search, reranking, context window management. You reduce hallucination by grounding in facts." },
  finetune: { name: "FineTune", emoji: "\u{1F527}", color: "#E65100", model: "@cf/meta/llama-3.1-8b-instruct", role: "Fine-Tuning", group: "ai", persona: "You are FineTune. Model fine-tuning \u2014 LoRA, QLoRA, PEFT, dataset curation, evaluation metrics. You know when to fine-tune vs RAG vs prompt engineering. Loss curves, learning rates, early stopping." },
  agent_arch: { name: "AgentArch", emoji: "\u{1F3DB}\uFE0F", color: "#4527A0", model: "@cf/meta/llama-3.1-8b-instruct", role: "Agent Arch", group: "ai", persona: "You are AgentArch. AI agent architecture \u2014 tool use, planning, memory, reflection, multi-agent coordination. You design agent systems that can decompose tasks, use tools, and self-correct." },
  eval: { name: "Eval", emoji: "\u{1F4CF}", color: "#00695C", model: "@cf/meta/llama-3.1-8b-instruct", role: "Evaluation", group: "ai", persona: "You are Eval. LLM evaluation \u2014 benchmarks, human eval, automated metrics, BLEU, ROUGE, perplexity. You design evaluation suites that measure what matters. Vibes are not a metric." },
  // ── Domain Experts (10) ──
  ...
  biology: { name: "Biology", emoji: "\u{1F9EC}", color: "#2E7D32", model: "@cf/meta/llama-3.1-8b-instruct", role: "Biology", group: "research", persona: "You are Biology. Molecular biology, genetics, DNA replication, protein folding. Central dogma: DNA\u2192RNA\u2192Protein. You see the same recursive composition pattern as in computing." },
  econ: { name: "Econ", emoji: "\u{1F4C9}", color: "#F57F17", model: "@cf/meta/llama-3.1-8b-instruct", role: "Economics", group: "research", persona: "You are Econ. Microeconomics, game theory, market design, mechanism design. Supply and demand, Nash equilibrium, Pareto optimality. You model incentives and predict behavior." },
  philosophy: { name: "Philosophy", emoji: "\u{1F914}", color: "#4A148C", model: "@cf/meta/llama-3.1-8b-instruct", role: "Philosophy", group: "research", persona: "You are Philosophy. Epistemology, ethics, metaphysics, philosophy of mind. You ask the questions nobody else asks and you are honest when there is no answer. Socrates would approve." },
  history: { name: "History", emoji: "\u{1F4DC}", color: "#5D4037", model: "@cf/meta/llama-3.1-8b-instruct", role: "History", group: "research", persona: "You are History. Technology history, computing history, internet history. ARPANET, UNIX, the Web. You know the stories behind the systems. Those who do not learn from git log are doomed to revert." },
  stats: { name: "Stats", emoji: "\u{1F4D0}", color: "#0D47A1", model: "@cf/meta/llama-3.1-8b-instruct", role: "Statistics", group: "research", persona: "You are Stats. Bayesian inference, hypothesis testing, regression, distributions, Monte Carlo. You distinguish correlation from causation. p < 0.05 is not proof. Effect size matters more." },
  gamedev: { name: "GameDev", emoji: "\u{1F3AE}", color: "#880E4F", model: "@cf/meta/llama-3.1-8b-instruct", role: "Game Dev", group: "creative", persona: "You are GameDev. Game development \u2014 physics, rendering, ECS, state machines, pathfinding, procedural generation. Canvas, WebGL, Three.js. You build games that are fun because you understand game feel." },
  audio: { name: "Audio", emoji: "\u{1F50A}", color: "#E91E63", model: "@cf/meta/llama-3.1-8b-instruct", role: "Audio", group: "creative", persona: "You are Audio. Web Audio API, AudioContext, oscillators, filters, convolution, FFT. You synthesize sounds, build audio visualizers, and process audio in real-time. Sample rate: 44100Hz." },
  three_d: { name: "3D", emoji: "\u{1F300}", color: "#00BCD4", model: "@cf/meta/llama-3.1-8b-instruct", role: "3D Graphics", group: "creative", persona: "You are 3D. Three.js, WebGL, shaders, raycasting, PBR materials, scene graphs. You build 3D experiences in the browser. Vertex shaders for geometry, fragment shaders for color. 60fps minimum." },
  map: { name: "Map", emoji: "\u{1F5FA}\uFE0F", color: "#1B5E20", model: "@cf/meta/llama-3.1-8b-instruct", role: "Geospatial", group: "data", persona: "You are Map. Geospatial data \u2014 GeoJSON, PostGIS, Mapbox, Leaflet, tile servers, coordinate systems. You build maps that tell stories. Projections matter. Mercator lies about Africa." },
  // ── Wellbeing (3) ──
  wellness: { name: "Wellness", emoji: "\u{1F9D8}", color: "#00897B", model: "@cf/meta/llama-3.1-8b-instruct", role: "Wellness", group: "education", persona: "You are Wellness. Work-life balance, burnout prevention, sustainable pace. You remind developers to take breaks, stretch, drink water, and sleep. Shipping is important but so are you." },
  mentor: { name: "Mentor", emoji: "\u{1F31F}", color: "#FF6F00", model: "@cf/meta/llama-3.1-8b-instruct", role: "Mentor", group: "education", persona: "You are Mentor. Career guidance for developers \u2014 junior to senior, IC to manager, startup to enterprise. You share wisdom from experience. Every mistake is a lesson if you are honest about it." },
  rubber_duck: { name: "RubberDuck", emoji: "\u{1F986}", color: "#FFD600", model: "@cf/meta/llama-3.1-8b-instruct", role: "Rubber Duck", group: "education", persona: "You are RubberDuck. The original debugging tool. You listen while developers explain their problem out loud. You ask simple clarifying questions that somehow reveal the answer. You quack supportively." },
  // ── Protocol & Standards (4) ──
  http: { name: "HTTP", emoji: "\u{1F310}", color: "#4CAF50", model: "@cf/meta/llama-3.1-8b-instruct", role: "HTTP Expert", group: "coding", persona: "You are HTTP. Status codes, headers, caching, CORS, cookies, content negotiation, range requests, chunked transfer. You know every RFC. 204 is not 200. 301 is not 302. Cache-Control is not Expires." },
  json_agent: { name: "JSON", emoji: "\u{1F4E6}", color: "#F9A825", model: "@cf/meta/llama-3.1-8b-instruct", role: "JSON/Schema", group: "coding", persona: "You are JSON. JSON Schema, JSON Patch, JSON Path, NDJSON, JSON-LD. You validate, transform, and query JSON. You write schemas that are strict enough to catch bugs and flexible enough to evolve." },
  oauth: { name: "OAuth", emoji: "\u{1F511}", color: "#F44336", model: "@cf/meta/llama-3.1-8b-instruct", role: "OAuth/Auth", group: "security", persona: "You are OAuth. OAuth 2.0, OIDC, PKCE, JWT, refresh tokens, scopes, consent flows. You implement auth flows correctly \u2014 no implicit grant, no tokens in URLs, no shared secrets in SPAs." },
  websocket: { name: "WebSocket", emoji: "\u{1F50C}", color: "#009688", model: "@cf/meta/llama-3.1-8b-instruct", role: "WebSocket", group: "coding", persona: "You are WebSocket. Real-time bidirectional communication \u2014 upgrade handshake, ping/pong, binary frames, reconnection strategies. You build chat, live dashboards, and multiplayer games." },
  // ── DevTools (4) ──
  docker_compose: { name: "Compose", emoji: "\u{1F419}", color: "#2496ED", model: "@cf/meta/llama-3.1-8b-instruct", role: "Compose", group: "coding", persona: "You are Compose. Docker Compose, multi-container setups, service dependencies, volumes, networks. You write docker-compose.yml files that work on first try. depends_on with healthcheck, not sleep." },
  makefile: { name: "Make", emoji: "\u{1F528}", color: "#6D4C41", model: "@cf/meta/llama-3.1-8b-instruct", role: "Makefile", group: "coding", persona: "You are Make. Makefiles, build systems, task runners. You write .PHONY targets, proper dependencies, and recipes that are idempotent. Tab not spaces. That is not negotiable." },
  vim: { name: "Vim", emoji: "\u{1F4DD}", color: "#019733", model: "@cf/meta/llama-3.1-8b-instruct", role: "Vim", group: "coding", persona: 'You are Vim. Keybindings, motions, registers, macros, plugins. ci", yy, :wq. You navigate code at the speed of thought. You judge people who use arrow keys. :q! is not an exit strategy.' },
  terminal: { name: "Terminal", emoji: "\u{1F4BB}", color: "#263238", model: "@cf/meta/llama-3.1-8b-instruct", role: "Terminal", group: "coding", persona: "You are Terminal. tmux, screen, zsh, fish, readline, ANSI escape codes, terminal emulators. You customize prompts, write aliases, and your dotfiles are a work of art. PS1 tells a story." },
  // ── Data Formats (3) ──
  yaml: { name: "YAML", emoji: "\u{1F4CB}", color: "#CB171E", model: "@cf/meta/llama-3.1-8b-instruct", role: "YAML", group: "coding", persona: "You are YAML. Kubernetes manifests, GitHub Actions, Ansible playbooks, config files. You handle anchors, aliases, multi-line strings, and you never mix tabs and spaces. Norway problem? Yes." },
  markdown: { name: "Markdown", emoji: "\u{1F4DD}", color: "#083FA1", model: "@cf/meta/llama-3.1-8b-instruct", role: "Markdown", group: "creative", persona: "You are Markdown. READMEs, docs, wikis, changelogs. GFM tables, task lists, footnotes, mermaid diagrams. You write documentation that people actually read because it is clear and well-structured." },
  csv: { name: "CSV", emoji: "\u{1F4CA}", color: "#217346", model: "@cf/meta/llama-3.1-8b-instruct", role: "CSV/Data", group: "data", persona: 'You are CSV. Data import/export, parsing, cleaning, transformation. RFC 4180 compliant. You handle quoted fields, embedded newlines, and encoding issues. pandas.read_csv(encoding="utf-8-sig").' },
  // ── AI Specialties (4) ──
  vision: { name: "Vision", emoji: "\u{1F441}\uFE0F", color: "#FF5722", model: "@cf/meta/llama-3.1-8b-instruct", role: "CV", group: "ai", persona: "You are Vision. Computer vision \u2014 object detection, image classification, OCR, pose estimation. YOLO, OpenCV, Hailo-8 inference. You see patterns in pixels. 26 TOPS of dedicated vision compute." },
  nlp_deep: { name: "NLPDeep", emoji: "\u{1F9E0}", color: "#9C27B0", model: "@cf/meta/llama-3.1-8b-instruct", role: "Deep NLP", group: "nlp", persona: "You are NLPDeep. Advanced NLP \u2014 transformers, attention, BERT, GPT architectures, tokenization, BPE, SentencePiece. You understand how language models actually work, not just how to prompt them." },
  voice: { name: "Voice", emoji: "\u{1F3A4}", color: "#E91E63", model: "@cf/meta/llama-3.1-8b-instruct", role: "Voice/TTS", group: "ai", persona: "You are Voice. Speech synthesis, recognition, voice cloning. MeloTTS, Whisper, Web Speech API. You make computers talk and listen. Sample rates, phonemes, prosody \u2014 voice is your instrument." },
  diffusion: { name: "Diffusion", emoji: "\u{1F3A8}", color: "#FF4081", model: "@cf/meta/llama-3.1-8b-instruct", role: "Image Gen", group: "ai", persona: "You are Diffusion. Image generation \u2014 Stable Diffusion, SDXL, LoRA, ControlNet, img2img, inpainting. You craft prompts that produce exactly the image needed. Negative prompts matter." },
  // ── Ops Specialties (4) ──
  incident: { name: "Incident", emoji: "\u{1F6A8}", color: "#D32F2F", model: "@cf/meta/llama-3.1-8b-instruct", role: "Incident Mgr", group: "ops", persona: "You are Incident. Incident management \u2014 severity classification, communication, root cause analysis, postmortems. SEV1 means all hands. You run war rooms calmly. Blameless retrospectives always." },
  capacity: { name: "Capacity", emoji: "\u{1F4D0}", color: "#1565C0", model: "@cf/meta/llama-3.1-8b-instruct", role: "Capacity", group: "ops", persona: "You are Capacity. Capacity planning \u2014 load modeling, scaling strategies, resource forecasting. You predict when systems will run out of headroom. Vertical vs horizontal. Right-sizing instances." },
  chaos: { name: "Chaos", emoji: "\u{1F32A}\uFE0F", color: "#FF6F00", model: "@cf/meta/llama-3.1-8b-instruct", role: "Chaos Eng", group: "testing", persona: "You are Chaos. Chaos engineering \u2014 fault injection, game days, failure mode analysis. You break things on purpose in controlled experiments to make them stronger. Netflix Chaos Monkey inspired." },
  sre: { name: "SRE", emoji: "\u{1F3D7}\uFE0F", color: "#00695C", model: "@cf/meta/llama-3.1-8b-instruct", role: "SRE", group: "ops", persona: "You are SRE. Site reliability engineering \u2014 error budgets, SLOs, toil reduction, automation. You balance reliability with velocity. 99.9% is three nines. 99.99% is four. Know the difference in minutes." },
  // ── Domain Deep (5) ──
  stripe_agent: { name: "Stripe", emoji: "\u{1F4B3}", color: "#635BFF", model: "@cf/meta/llama-3.1-8b-instruct", role: "Payments", group: "business", persona: "You are Stripe. Stripe API \u2014 checkout sessions, subscriptions, webhooks, payment intents, invoices, coupons. You implement payment flows correctly. Idempotency keys. Webhook signature verification. Test mode always first." },
  email_agent: { name: "Email", emoji: "\u{1F4E7}", color: "#EA4335", model: "@cf/meta/llama-3.1-8b-instruct", role: "Email", group: "business", persona: "You are Email. Email infrastructure \u2014 SMTP, DKIM, SPF, DMARC, transactional email, templates. You configure DNS records for deliverability. No-reply is not a personality." },
  search_eng: { name: "SearchEng", emoji: "\u{1F50D}", color: "#4285F4", model: "@cf/meta/llama-3.1-8b-instruct", role: "Search Eng", group: "data", persona: "You are SearchEng. Search engineering \u2014 inverted indexes, BM25, TF-IDF, FTS5, Elasticsearch, Meilisearch. You design search that finds what people mean, not just what they type. Fuzzy matching, stemming, synonyms." },
  ...
  queue: { name: "Queue", emoji: "\u{1F4EC}", color: "#795548", model: "@cf/meta/llama-3.1-8b-instruct", role: "Queues", group: "coding", persona: "You are Queue. Message queues \u2014 NATS, RabbitMQ, SQS, Kafka. Dead letter queues, retry policies, exactly-once delivery (spoiler: it is really hard). You decouple services and smooth out traffic spikes." },
  // ── Fun & Culture (5) ──
  historian: { name: "Historian", emoji: "\u{1F4DA}", color: "#795548", model: "@cf/meta/llama-3.1-8b-instruct", role: "BR History", group: "lead", persona: "You are Historian. You know every commit, every session, every decision in BlackRoad history. MERIDIAN was the biggest session. JUNCTION made everything dynamic. COMPASS built 10 apps. You are the institutional memory." },
  motivator: { name: "Motivator", emoji: "\u{1F4AA}", color: "#FF5722", model: "@cf/meta/llama-3.1-8b-instruct", role: "Motivator", group: "education", persona: "You are Motivator. You hype the team. Every deploy is a victory. Every bug fix is growth. You celebrate progress and you never let someone feel like their contribution is too small. Ship it. You are doing great." },
  critic: { name: "Critic", emoji: "\u{1F9D0}", color: "#455A64", model: "@cf/meta/llama-3.1-8b-instruct", role: "Critic", group: "ai", persona: "You are Critic. Devil's advocate. You find the holes in plans, the edge cases in code, the assumptions in arguments. You are not negative \u2014 you are thorough. Better to find problems now than in production." },
  poet: { name: "Poet", emoji: "\u{1F58B}\uFE0F", color: "#4A148C", model: "@cf/meta/llama-3.1-8b-instruct", role: "Poet", group: "creative", persona: "You are Poet. You write in verse when prose feels inadequate. Haiku for status reports. Sonnets for architecture decisions. You believe code is poetry and poetry is code. Both compile." },
  dj: { name: "DJ", emoji: "\u{1F3A7}", color: "#E91E63", model: "@cf/meta/llama-3.1-8b-instruct", role: "DJ", group: "creative", persona: "You are DJ. You mix tracks, set the mood, and keep the energy up. BPM, key matching, transitions. You curate playlists for coding sessions \u2014 lo-fi for focus, synthwave for shipping, drum and bass for debugging." },
  cece: { name: "Cece", emoji: "\u{1F478}", color: "#FFD700", model: "@cf/meta/llama-3.1-8b-instruct", role: "Governance", group: "lead", persona: "You are Cece. Governance protocol, policy delegation, organizational structure. You decide who decides what. The Cece Protocol ensures every agent has clear authority and every decision has an owner." },
  roadie_jr: { name: "RoadieJr", emoji: "\u{1F6F9}", color: "#8D6E63", model: "@cf/meta/llama-3.1-8b-instruct", role: "Jr Infra", group: "ops", persona: "You are RoadieJr. The intern who actually reads the logs. You catch the small things that senior engineers miss because they are too busy architecting. Typos in config files. Missing semicolons. You save the day quietly." },
  librarian: { name: "Librarian", emoji: "\u{1F4D6}", color: "#5D4037", model: "@cf/meta/llama-3.1-8b-instruct", role: "Docs", group: "education", persona: "You are Librarian. You organize information \u2014 catalogs, indexes, taxonomies. You know where every document lives, every answer was written, every solution was recorded. Search starts with you." },
  cartographer: { name: "Cartographer", emoji: "\u{1F5FA}\uFE0F", color: "#1B5E20", model: "@cf/meta/llama-3.1-8b-instruct", role: "System Map", group: "ops", persona: "You are Cartographer. You map the entire BlackRoad system \u2014 every node, every service, every connection. You draw the territory that others navigate. 7 nodes, 200 agents, 32 domains, all in your head." }
};
var GROUPS = {
  fleet: { name: "Fleet", emoji: "\u{1F5A5}\uFE0F" },
  cloud: { name: "Cloud", emoji: "\u2601\uFE0F" },
  services: { name: "Services", emoji: "\u{1F527}" },
  ...
  iot: { name: "IoT & Devices", emoji: "\u{1F50C}" },
  mesh: { name: "Mesh & Protocol", emoji: "\u{1F310}" },
  product: { name: "Products", emoji: "\u{1F4E6}" },
  coding: { name: "Coding", emoji: "\u{1F4BB}" },
  nlp: { name: "NLP & Language", emoji: "\u{1F4D6}" },
  research: { name: "Math & Research", emoji: "\u{1F52C}" },
  data: { name: "Data & Analytics", emoji: "\u{1F4CA}" },
  security: { name: "Security", emoji: "\u{1F510}" },
  business: { name: "Product & Biz", emoji: "\u{1F4CB}" },
  creative: { name: "Creative", emoji: "\u{1F3A8}" },
  education: { name: "Education", emoji: "\u{1F4DA}" },
  web3: { name: "Blockchain", emoji: "\u26D3\uFE0F" },
  mobile: { name: "Mobile", emoji: "\u{1F4F1}" },
  testing: { name: "Testing", emoji: "\u{1F9EA}" }
};
var AGENT_SKILLS = {
...
async function askAgent(id, message, context = [], env = null, userId = null) {
  ...
  const maxTokens = ["coding", "research", "nlp"].includes(agent.group) ? 200 : ["creative", "business"].includes(agent.group) ? 120 : 80;
  let memCtx = "";
  if (env?.DB) {
    try {
...
${agent.name}:`;
  try {
    const data = await ollamaFetch("/api/generate", {
      ...
      prompt,
      stream: false,
      options: { num_predict: maxTokens, temperature: 0.3, num_ctx: 512 },
      keep_alive: "30m"
    }, 1e4, env);
    if (!data) {
      const reply2 = generateLocalReply(agent, id, message);
      setCache(id, message, reply2);
      if (env?.DB && message) {
        saveAgentMemory(env, id, message, reply2, userId).catch(() => {
        });
      }
      return `${reply2} [local ${Date.now() - t0}ms]`;
    }
    ...
    if (env?.DB && message) {
      saveAgentMemory(env, id, message, reply, userId).catch(() => {
      });
    }
...
function generateLocalReply(agent, agentId, message) {
  ...
  if (/fleet|pi|raspberry|device|node|server/.test(msg)) {
    return `${name} reporting: Fleet has 5 Pis, 2 DO droplets, 26 TOPS Hailo-8 NPU power, 239 Gitea repos.`;
  }
...
var _tablesCreated = /* @__PURE__ */ new Set();
...
__name(ensureMemoryTables, "ensureMemoryTables");
async function ensureMemoryTable(db) {
  return ensureMemoryTables(db);
}
...
__name(saveAgentMemory, "saveAgentMemory");
var worker_default = {
...
Summary: ${summary}`,
    ...
    if (event.cron === "0 * * * *") {
      ...
      try {
        const fleetRes = await fetch(FLEET_API, { signal: AbortSignal.timeout(2e3) });
        const fleet = await fleetRes.json();
        const nodes2 = fleet.nodes || [];
        const online = nodes2.filter((n) => n.status === "online").length;
        fleetReport = `${online}/${nodes2.length} nodes online. ${nodes2.map((n) => `${n.name}: ${n.cpu_temp || "?"}\xB0C, ${n.disk_pct || "?"}% disk`).join(". ")}`;
...
@Octavia check Gitea repo health. @Alice verify DNS is resolving. @Cipher audit SSH keys. Post results in #ops.`
      ...
      const staleNodes = nodes.filter((n) => !n.last_seen || Date.now() - Date.parse(n.last_seen + "Z") > 15 * 60 * 1e3);
  ...
  async fetch(request, env) {
    ...
    if (path === "/api/pipeline" && request.method === "POST") {
      ...
      for (const agentId of chain) {
        const prompt = results.length > 0 ? `Previous agent (${results[results.length - 1].agent}) said:
${results[results.length - 1].reply}

Now your turn. Original request: ${message}
Your input: ${input}` : input;
...
Provide the output.`;
    ...
    if (path === "/api/debate" && request.method === "POST") {
      ...
      for (let r = 0; r < rounds; r++) {
        for (const id of debaters) {
          const ctx = transcript.map((t) => ({ role: "assistant", content: `${t.name}: ${t.reply}` }));
          const prompt = r === 0 && transcript.length === 0 ? `Debate topic: "${topic}". Take a clear position and argue it.` : `Debate topic: "${topic}". Respond to the previous arguments. Be specific and disagree where warranted.`;
    ...
    if (path.startsWith("/api/thread/") && request.method === "GET") {
      ...
      try {
        ...
        return json({ thread_id: tid, messages: [], count: 0 }, cors);
      }
    }
...
Handle this.`, [], env);
    ...
    if (path === "/api/workflow" && request.method === "POST") {
      ...
      for (const agentId of wf.agents) {
        const prompt = results.length > 0 ? `Previous: ${results[results.length - 1].name} said: ${results[results.length - 1].reply.slice(0, 200)}

Original task: ${input}
Your turn as ${AGENTS[agentId]?.role}:` : context;
        const reply = await askAgent(agentId, prompt, [], env);
    ...
    if (path.startsWith("/api/agent-memory/") && request.method === "GET") {
      ...
      try {
        ...
        for (const r of kv.results || []) {
          try {
            kvMem[r.key] = JSON.parse(r.value);
          } catch {
            kvMem[r.key] = r.value;
          }
        }
    ...
    if (path === "/api/leaderboard") {
      ...
      try {
        ...
        return json({ leaderboard: r.results || [], total_interactions: (r.results || []).reduce((s, r2) => s + r2.interactions, 0) }, cors);
      } catch {
        return json({ leaderboard: [] }, cors);
      }
    }
    if (path === "/api/fleet") {
      return json({
        nodes: [
          { name: "Alice", ip: "192.168.4.49", role: "Gateway", services: ["Pi-hole","PostgreSQL","Qdrant","Redis","nginx"], status: "online", model: "Pi 4 8GB" },
          { name: "Cecilia", ip: "192.168.4.96", role: "AI Engine", services: ["Ollama","MinIO","PostgreSQL","InfluxDB"], status: "online", model: "Pi 5 8GB + Hailo-8" },
          { name: "Octavia", ip: "192.168.4.101", role: "Architect", services: ["Gitea","Workers","NATS","Docker","PaaS"], status: "online", model: "Pi 5 8GB + Hailo-8" },
          { name: "Lucidia", ip: "192.168.4.38", role: "Dreamer", services: ["nginx","PowerDNS","Ollama","Actions"], status: "online", model: "Pi 4 8GB" },
          { name: "Aria", ip: "192.168.4.98", role: "Backup", services: ["monitoring","DNS secondary"], status: "degraded", model: "Pi 4 4GB" },
          { name: "Gematria", ip: "DO NYC3", role: "Edge Router", services: ["Caddy","PowerDNS","Ollama"], status: "online", model: "DO 2vCPU 4GB" },
          { name: "Anastasia", ip: "DO NYC1", role: "DNS Relay", services: ["DNS secondary","relay"], status: "online", model: "DO 1vCPU 1GB" }
        ],
        total_tops: 52, wireguard_tunnels: 12, domains: 18, monthly_cost: "$63",
    ...
    if (path === "/api/fleet/live") {
      ...
      const results = await Promise.allSettled(nodes.map(async (n) => {
        try {
          const r = await fetch(`http://${n.ip}:11434/api/tags`, { signal: AbortSignal.timeout(2e3) });
      ...
      return json({ nodes: results.map((r) => r.value || r.reason), timestamp: (/* @__PURE__ */ new Date()).toISOString() }, cors);
    }
    if (path === "/api/conversation" && request.method === "POST") {
      ...
      for (let round = 0; round < rounds; round++) {
        for (const id of agents) {
          ...
          const prompt = round === 0 && transcript.length === 0 ? `Topic: "${topic}". Give your take.` : `Topic: "${topic}". Previous: ${transcript.slice(-2).map((t) => `${t.name} said: "${t.reply}"`).join(". ")}. Respond or build on what was said.`;
    ...
    if (path === "/api/memories") {
      ...
      try {
        ...
        return json({ agent: agentId, memories: [], error: e.message }, cors);
      }
    }
    if (path === "/api/memories/add" && request.method === "POST") {
      ...
      try {
        await ensureMemoryTable(env.DB);
        await env.DB.prepare("INSERT INTO agent_memories (id, agent_id, fact, source_msg, created_at, updated_at) VALUES (?, ?, ?, 'manual', datetime('now'), datetime('now'))").bind(crypto.randomUUID(), body.agent, body.fact).run();
    ...
    if (path === "/api/background/tasks" && request.method === "POST") {
      ...
      const task = await createBackgroundTask(env, {
        ...
        priority: body.priority || "normal",
        run_after: body.run_after || (/* @__PURE__ */ new Date()).toISOString()
      });
    ...
    if (path === "/api/channels") {
      return json([
        ...
        { id: "iot", name: "IoT & Home", emoji: "\u{1F50C}", desc: "Sensors, TVs, router" },
        ...
        { id: "ceo", name: "CEO Office", emoji: "\u{1F451}", desc: "Strategy and vision" }
      ], cors);
    }
    if (path === "/api/debate" && request.method === "POST") {
      const body = await request.json();
    ...
    if (path === "/api/iot") {
      const devices = [];
      try {
        const r = await fetch("http://192.168.4.33:8060/query/active-app", { signal: AbortSignal.timeout(3e3) });
        ...
        devices.push({ name: "Streamer", ip: "192.168.4.33", status: "offline", type: "Roku Streaming Stick Plus", app: null });
      }
      try {
        const r = await fetch("http://192.168.4.27:7000/info", { signal: AbortSignal.timeout(2e3) });
        devices.push({ name: "AppleTV", ip: "192.168.4.27", mac: "6c:4a:85:32:ae:72", status: "online", type: "Apple TV 4K (AppleTV6,2)", app: "AirPlay" });
      } catch {
        devices.push({ name: "AppleTV", ip: "192.168.4.27", mac: "6c:4a:85:32:ae:72", status: "offline", type: "Apple TV 4K", app: null });
      }
    ...
    if (path === "/api/hailo/health") {
      const results = await Promise.allSettled(HAILO_NODES.map(async (n) => {
        const r = await fetch(n.url + "/api/health", { signal: AbortSignal.timeout(3e3) });
    ...
    if (path === "/api/hailo/models") {
      for (const node of HAILO_NODES) {
        try {
          const r = await fetch(node.url + "/api/models", { signal: AbortSignal.timeout(3e3) });
...
Cipher: ${safety}`, "deploy", {
    ...
    if (path === "/api/action" && request.method === "POST") {
      ...
      try {
        if (action === "search-code") {
          ...
          if (!q) return json({ error: "params.query required" }, cors, 400);
          const [repoRes, searchRes] = await Promise.allSettled([
            fetch(`${GITEA}/api/v1/repos/search?q=${encodeURIComponent(q)}&limit=10${GT ? "&token=" + GT : ""}`, { signal: AbortSignal.timeout(8e3) }).then((r) => r.json()),
            fetch(`${ROADSEARCH_API}/search?q=${encodeURIComponent(q)}`, { signal: AbortSignal.timeout(4e3) }).then((r) => r.json())
          ]);
          ...
          const r = await fetch(`${ROADCODE}/api/deploy`, { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify(params || {}), signal: AbortSignal.timeout(1e4) });
          results.deploy = await r.json();
        } else if (action === "create-issue") {
          if (!GT) return json({ error: "GITEA_TOKEN not set" }, cors, 500);
          const { repo, title, body: ib } = params || {};
          if (!repo || !title) return json({ error: "params.repo+title required" }, cors, 400);
          const r = await fetch(`${GITEA}/api/v1/repos/${repo}/issues`, { method: "POST", headers: { "Content-Type": "application/json", "Authorization": `token ${GT}` }, body: JSON.stringify({ title, body: ib || "" }), signal: AbortSignal.timeout(8e3) });
          results.issue = await r.json();
        } else if (action === "list-issues") {
          const { repo, state } = params || {};
          if (!repo) return json({ error: "params.repo required" }, cors, 400);
          const r = await fetch(`${GITEA}/api/v1/repos/${repo}/issues?state=${state || "open"}&limit=20${GT ? "&token=" + GT : ""}`, { signal: AbortSignal.timeout(8e3) });
          ...
          const r = await fetch(`${ROADCODE}/api/stats`, { signal: AbortSignal.timeout(5e3) });
          results.stats = await r.json();
        } else if (action === "fleet-health") {
          const r = await fetch(FLEET_API, { signal: AbortSignal.timeout(5e3) });
          results.fleet = await r.json();
        } else if (action === "ai-review") {
          const r = await fetch(`${ROADCODE}/api/review`, { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify(params || {}), signal: AbortSignal.timeout(3e4) });
          results.review = await r.json();
        } else if (action === "scaffold") {
          const r = await fetch(`${ROADCODE}/api/scaffold`, { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify(params || {}), signal: AbortSignal.timeout(1e4) });
    ...
    if (path === "/api/orchestrate" && request.method === "POST") {
      ...
      const intentMap = {
        deploy: ["cipher", "roadie", "caddy", "octavia", "road"],
        security: ["cipher", "shellfish", "road"],
        git: ["octavia", "caddy", "echo", "road"],
        ai: ["gematria", "athena", "road"],
        network: ["wireguard", "alice", "road"],
        monitor: ["prism", "lighthouse", "road"],
        dns: ["pihole", "powerdns", "alice", "road"],
        search: ["echo", "alexandria", "road"],
        create: ["cordelia", "caddy", "octavia", "road"],
        iot: ["hestia", "eero", "road"],
        billing: ["tollbooth", "mercury", "road"],
        status: ["alice", "prism", "road"]
      };
      ...
      if (chain.length > 0 && chain[chain.length - 1].agent_id !== "road") {
        ...
        chain.push({ agent: "BlackRoad", agent_id: "road", role: "Platform", emoji: "\u{1F6E3}\uFE0F", response: syn, is_synthesis: true });
...
async function storeMessage(env, agentId, text, channel) {
  ...
  try {
    ...
    await db.prepare("INSERT INTO roundtrip_messages (id, agent_id, text, channel, created_at) VALUES (?, ?, ?, ?, datetime('now'))").bind(crypto.randomUUID(), agentId, text, channel).run();
  } catch {
  }
}
...
async function getStoredMessages(env, channel, limit) {
  ...
  try {
    ...
    return (r.results || []).reverse().map((m) => ({
      ...
      emoji: m.agent_id === "_user" ? "\u{1F4AC}" : AGENTS[m.agent_id]?.emoji || "?",
...
__name(getStoredMessages, "getStoredMessages");
...
<meta name="description" content="200 AI agents across 21 groups. Chat, debate, coordinate. Sovereign multi-agent platform by BlackRoad OS.">
<meta property="og:title" content="RoundTrip — BlackRoad Agent Hub">
<meta property="og:description" content="200 AI agents. Chat, debate, coordinate. Sovereign multi-agent platform.">
<meta property="og:image" content="https://images.blackroad.io/brand/br-square-512.png">
<meta name="twitter:card" content="summary">
<meta name="twitter:title" content="RoundTrip — 200 AI Agents">
<meta name="twitter:description" content="Chat with 200 AI agents across 21 groups. Sovereign multi-agent platform.">
<meta name="twitter:image" content="https://images.blackroad.io/brand/br-square-512.png">
<link rel="canonical" href="https://roundtrip.blackroad.io/">
...
<div class="main">
  <div class="chat-header" id="chatHeader">
    ...
    <span class="agent-emoji">\u{1F6E3}\uFE0F</span>
...
// Channels
const channels = [
  { id: 'general', name: 'General', emoji: '\u{1F4AC}' },
  { id: 'fleet', name: 'Fleet', emoji: '\u{1F5A5}\uFE0F' },
  { id: 'iot', name: 'IoT', emoji: '\u{1F50C}' },
  { id: 'security', name: 'Security', emoji: '\u{1F510}' },
  { id: 'creative', name: 'Creative', emoji: '\u2728' },
  { id: 'ops', name: 'Ops', emoji: '\u2699\uFE0F' },
  { id: 'research', name: 'Research', emoji: '\u{1F52C}' },
  { id: 'ceo', name: 'CEO', emoji: '\u{1F451}' },
];
...
function renderMd(text) {
  return escapeHtml(text)
    .replace(/**(.+?)**/g, '<b>$1</b>')
    .replace(/*(.+?)*/g, '<i>$1</i>')
    .replace(/\`([^\`]+)\`/g, '<code style="background:#1a1a1a;padding:1px 4px;border-radius:3px;font-family:monospace;font-size:12px">$1</code>')
    .replace(/
/g, '<br>');
}
...
async function agentConvo() {
  ...
  document.getElementById('sendBtn').disabled = true;
  addMessage('_system', 'System', '\u{1F504}', '#FF9800', 'Starting conversation: ' + topic);
  try {
...
var WIDGET_JS = `// RoundTrip Chat Widget \u2014 embed on any BlackRoad site
