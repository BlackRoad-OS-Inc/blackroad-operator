#!/usr/bin/env python3
"""analytics-blackroad MCP Bridge."""
...
AUTH_TOKEN = os.getenv(
    "MCP_BRIDGE_TOKEN",
    "blackroad-analytics-blackroad-bridge-token",
)
MEMORY_DIR = Path.home() / "blackroad" / "memory" / "analytics-blackroad"
MEMORY_DIR.mkdir(parents=True, exist_ok=True)

app = FastAPI(title="analytics-blackroad MCP Bridge")
app.add_middleware(
    CORSMiddleware,
    allow_origins=["*"],
...
@app.get("/")
async def root():
    return {"service": "analytics-blackroad MCP Bridge", "host": platform.node()}
