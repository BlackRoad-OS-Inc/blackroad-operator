...
AUTH_TOKEN = os.getenv(
    "MCP_BRIDGE_TOKEN",
    "blackroad-auth-blackroad-bridge-token",
)
MEMORY_DIR = Path.home() / "blackroad" / "memory" / "auth-blackroad"
MEMORY_DIR.mkdir(parents=True, exist_ok=True)

app = FastAPI(title="auth-blackroad MCP Bridge")
app.add_middleware(
    CORSMiddleware,
    allow_origins=["*"],
...
@app.get("/")
async def root():
    return {"service": "auth-blackroad MCP Bridge", "host": platform.node()}
