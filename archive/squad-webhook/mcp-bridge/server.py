...
AUTH_TOKEN = os.getenv(
    "MCP_BRIDGE_TOKEN",
    "blackroad-squad-webhook-bridge-token",
)
MEMORY_DIR = Path.home() / "blackroad" / "memory" / "squad-webhook"
MEMORY_DIR.mkdir(parents=True, exist_ok=True)

app = FastAPI(title="squad-webhook MCP Bridge")
app.add_middleware(
    CORSMiddleware,
    allow_origins=["*"],
...
@app.get("/")
async def root():
    return {"service": "squad-webhook MCP Bridge", "host": platform.node()}
