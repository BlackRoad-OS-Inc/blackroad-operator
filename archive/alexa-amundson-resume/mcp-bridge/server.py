#!/usr/bin/env python3
"""alexa-amundson-resume MCP Bridge."""
...
AUTH_TOKEN = os.getenv(
    "MCP_BRIDGE_TOKEN",
    "blackroad-alexa-amundson-resume-bridge-token",
)
MEMORY_DIR = Path.home() / "blackroad" / "memory" / "alexa-amundson-resume"
MEMORY_DIR.mkdir(parents=True, exist_ok=True)

app = FastAPI(title="alexa-amundson-resume MCP Bridge")
app.add_middleware(
    CORSMiddleware,
    allow_origins=["*"],
...
@app.get("/")
async def root():
    return {"service": "alexa-amundson-resume MCP Bridge", "host": platform.node()}
