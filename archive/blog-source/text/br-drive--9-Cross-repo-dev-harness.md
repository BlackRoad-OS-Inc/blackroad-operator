...
Classify your repos at a high level (using the list you just exposed).

Propose a blackroad-os-master layout to orchestrate them.

Give you concrete files: README.md + scripts/clone-core.sh + scripts/dev-stack-epics-1-3.sh you can drop in and go.
...
Start a local Postgres (via docker-compose or docker run) or assume DATABASE_URL points to a dev instance (Railway, Neon, etc.).

Apply DB migrations for:
...
Start gov-api (blackroad-os-api) on port 8000.

Start the web app (Next.js) on port 3000 with host-based routing for:
...
Teacher creates assignment at edu.blackroad.io

Student submits at homework.blackroad.io

Teacher reviews, Cece logs policies and ledger
